...
| Android ExoPlayer migration — plan 24 (**Active 2026-05-15; M1-M4 + M7 + M8 shipped same day** — Kotlin spike + `PlayerEngine` Dart abstraction in `packages/fluxora_core/lib/player/` + `ExoPlayerEngine` Dart talking to MethodChannel `dev.marshalx.fluxora/exo_player` + Kotlin module with `Player.Listener` events, 250ms position ticker, audio focus, becoming-noisy auto-pause, bearer-header injection without logging values, source-index→`(TrackGroup, formatIndex)` mapping + Media3 `MediaSessionService` for lockscreen / notification / BT transport + `ExoPlayerEngine.open()` state-reset emission + scrubber pin gate fix. Media3 1.10.1 (`-exoplayer`, `-exoplayer-hls`, `-ui`, `-session`). `_kEnableExoPlayerEngine = true` — Android sessions default to ExoPlayer; libmpv stays desktop + iOS engine + Android escape hatch via `_kForceMediaKitOnAndroid`. M5 (multi-audio device smoke) + M6 (HDR + tonemap) require operator device testing. M9 (tests + doc sweep + flag deletion + spike cleanup) is the final mechanical close-out. iOS deferred; multi-rendition HLS server output split into plan 25) | `docs/10_planning/24_player_audio_reliability_plan.md` |
