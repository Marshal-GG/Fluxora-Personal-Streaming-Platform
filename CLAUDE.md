...
| Client-side audio decoding — plan 21 (audio stream-copy for `{aac, ac3, eac3, opus, flac}` allowlist; fmp4 switch for non-AAC audio; separate `client_audio_codec_blocklist` table; new `POST /fallback-audio-transcode` endpoint for audio-only fallback; 128k → 256k AAC bitrate bump + `-ac` channel preservation on the re-encode paths that remain; M1-M5 ~12 h), drafted 2026-05-12 awaiting M1 sign-off | `docs/10_planning/21_client_audio_decoding.md` |
