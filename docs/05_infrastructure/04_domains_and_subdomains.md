...
| `fluxora-api.marshalx.dev` | Public entry to the home Fluxora server (REST + WS control plane; media never tunneled) | Cloudflare Tunnel `fluxora-home` → home PC `:8080` | Cloudflare DNS | **ON** | Cloudflare Universal SSL | v1 single-tenant Phases 1–5 complete 2026-05-01 (tunnel live + server CF middlewares + admin hardening + dual-base `ApiClient` + mobile pairing + desktop Remote Access UI). Phase 6 hardening operator-driven. Single-level subdomain (hyphen instead of dot) for Universal SSL coverage. |
