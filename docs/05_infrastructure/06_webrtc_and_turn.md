...
> **Last Updated:** 2026-05-09 (audit follow-up — env-var drift fixed: `webrtc_service.py::_ice_servers()` now reads `settings.fluxora_turn_*` directly, matching the canonical names declared in `config.py`. Only the `FLUXORA_TURN_*` env vars need to be set; the deprecated `WEBRTC_TURN_*` workaround is gone. +3 regression tests in `test_webrtc_ice_servers.py`.)
...
Restart the Fluxora server. When all three `FLUXORA_TURN_*` values are non-empty, the `RTCConfiguration` returned to clients includes the TURN entry — ICE will try direct, then STUN, then TURN.

`webrtc_service.py:_ice_servers()` logs `TURN server configured: turn:turn.fluxora.marshalx.dev:3478` at INFO when the values resolve, or drops to `No TURN server configured — using STUN only` at DEBUG when any of the three is empty.
