...
| **Localhost-only admin** | `GET/PATCH /api/v1/settings`, `POST /api/v1/auth/approve/{id}`, `POST /api/v1/auth/reject/{id}`, `GET /api/v1/auth/clients`, `GET /api/v1/orders`, `GET /api/v1/stream/sessions`, `POST /api/v1/info/restart`, `POST /api/v1/info/stop`, `POST /api/v1/groups`, `PATCH /api/v1/groups/{id}`, `DELETE /api/v1/groups/{id}`, `POST /api/v1/groups/{id}/members`, `DELETE /api/v1/groups/{id}/members/{cid}`, `GET /api/v1/profile`, `PATCH /api/v1/profile` | direct (loopback only) | **never reached over WAN** — `require_local_caller` 403s remote callers |
