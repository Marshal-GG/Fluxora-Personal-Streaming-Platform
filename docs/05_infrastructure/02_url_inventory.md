...
> **Last Updated:** 2026-05-20 (added `DELETE /api/v1/auth/clients/{client_id}` operator hard-delete — distinct from revoke; surfaced from the desktop Clients screen's Revoked-filter view as a prune action; FK cascades handle `group_members` + null out `stream_sessions.client_id`; records `client.delete` activity event).  2026-05-18 (folder-browser polish wave 3 sync — added `POST /api/v1/files/{file_id}/stream-test` (localhost-only dry-run for the "Stream test" affordance), `GET /api/v1/library/{library_id}/resolve-absolute` (URL-bar manual-typing fallback), `GET /api/v1/library/{library_id}/folder-size` + `POST /api/v1/library/{library_id}/index-file` + `POST /api/v1/library/{library_id}/scan-subtree` + `POST /api/v1/library/{library_id}/unindex-subtree` + `POST /api/v1/files/{file_id}/regenerate-thumbnail` (plan 28 Phase C/D + post-ship polish endpoints that were live in code but absent from this table); added `cataloged_size_bytes` to the `/browse` row description.).  2026-05-09 (deep-audit sync — added `POST /api/v1/info/support-bundle`, `GET /api/v1/files/{file_id}/content`, `PATCH /api/v1/auth/clients/me`, transcoding benchmark endpoints `POST /benchmark` + `GET /benchmark/progress` + `GET /benchmark/history` + `GET/DELETE /benchmark/history/{id}` that were live in code but missing from the inventory). 2026-05-08 (added `DELETE /api/v1/auth/clients/me` self-revoke for mobile sign-out — audit §17.3 #3; added `POST /api/v1/files/{file_id}/reset-progress` for the "Start over" affordance — streaming pipeline plan §4.10)
...
| `DELETE` | `/api/v1/auth/clients/{client_id}` | Localhost only | Operator hard-delete of a client row — physically removes from `clients` (FK cascades clean up `group_members`, null out `stream_sessions.client_id`). Distinct from revoke which only flips `status='rejected'`; this is the prune action on the desktop Clients screen's Revoked-filter view. Records a `client.delete` activity event |
...
| `POST` | `/api/v1/files/{file_id}/stream-test` | Localhost only | Dry-run sanity check for the desktop folder browser's "Stream test" affordance.  Verifies the file is reachable on disk + reports codec + the exact playback path `/stream/start` would route through (sidecar wins over source).  Does NOT spawn FFmpeg or INSERT a `stream_sessions` row.  Returns `{file_id, playback_path, codec, audio_codec, would_use_sidecar, ok}`. |
...
| `GET` | `/api/v1/library/{library_id}/browse` | Token or localhost | Filesystem-walk one directory under the library's `root_paths`.  Query: `?path=<relative>&show_hidden=<bool>`.  Returns dirs-first sorted listing + JOIN with `media_files.path` for `is_indexed` + `file_id` + `cataloged_size_bytes` (folder rows show sum-of-indexed-descendant-bytes without needing a separate folder-size walk).  Path-traversal-secured.  Powers the desktop folder browser.  Plan 27 post-ship. |
| `GET` | `/api/v1/library/{library_id}/folder-size` | Token or localhost | Authoritative on-disk subtree walk for the folder browser's "Compute size" affordance.  Counts EVERY file (indexed + un-indexed + hidden) via `os.walk(followlinks=False)`.  Query: `?path=<relative>`.  Returns `{library_id, relative_path, size_bytes, file_count}`. |
| `GET` | `/api/v1/library/{library_id}/resolve-absolute` | Token or localhost | URL-bar manual-typing fallback for the folder browser — take an absolute on-disk path + return the library-relative form under the matching `root_paths` entry.  Walks every root via `Path.is_relative_to` after `Path.resolve(strict=True)` (Windows: gives back the OS canonical case + UNC normalisation).  404 on path missing or not under any root.  Returns `{library_id, root_path, relative_path}`. |
| `POST` | `/api/v1/library/{library_id}/index-file` | Token or localhost | Insert a single `media_files` row for a file under one of the library's `root_paths` + enqueue a priority-10 thumbnail.  Idempotent against an existing row (returns `already_indexed=true`).  Best-effort TMDB enrichment (skipped for DVR-pattern stems).  Backs the folder browser's right-click "Index this file".  Plan 28 Phase C. |
| `POST` | `/api/v1/library/{library_id}/scan-subtree` | Token or localhost | Walk just the requested subdir under one of the library's `root_paths` instead of every root.  Query: `?path=<relative>`.  Routes through `_scan_library_locked` so the per-library lock still applies.  Backs the folder browser's right-click "Scan this folder".  Plan 28 Phase C. |
| `POST` | `/api/v1/library/{library_id}/unindex-subtree` | Token or localhost | Delete every `media_files` row whose path sits under the given subtree.  Files on disk are NEVER touched.  Cascades to `media_thumbnails` / `transcode_jobs` via existing FKs.  Backs the folder browser's right-click "Unindex this folder".  Returns `{library_id, files_removed}`.  Single-file unindex reuses `DELETE /files/{file_id}`. |
| `POST` | `/api/v1/files/{file_id}/regenerate-thumbnail` | Token or localhost | Reset one file's thumbnail row to `pending` + priority=10 + delete the cached JPEG.  Backs the folder browser's right-click "Regenerate thumbnail".  Plan 28 Phase C. |
