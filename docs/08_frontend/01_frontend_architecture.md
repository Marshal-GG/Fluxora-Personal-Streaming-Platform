...
└── features/
    ...
    │       └── screens/clients_screen.dart  # M4 redesign: PageHeader + 4 StatTiles (Active Streams reads from SystemStatsCubit since 2026-05-06) + search/filter row + 7-column table (IP cell reads c.lastIp; Current Stream reads c.activeSession.mediaTitle) + 300px detail panel (approve/reject/revoke wired; emerald _ActiveSessionBlock when activeSession non-null). **2026-05-20 sweep:** (a) status chip + status-filter + stat tile now read a `_LiveStatus {online, idle, offline}` derived from `last_seen` freshness (`< 2 min = online`, `< 15 min = idle`, else `offline`) instead of the old `is_trusted` bool which was an authorization flag, not a connectivity signal — phones that were authorized but offline were incorrectly showing as Online; (b) added an operator-driven hard-delete action — `DELETE /auth/clients/{id}` via `ClientsCubit.delete` — replaces the unclickable history icon on Revoked rows with a trash-can icon + `_confirmDelete` FluxGlassDialog; (c) per-column drag-resize via `_ResizableClientColumnDivider` writes `ClientColumn`-keyed widths to `_clientColumnWidths` + bumps a `ValueNotifier<int>` so all header/row cells rebuild on the same frame; defaults sized so the seven columns fit the typical viewport without horizontal scroll (`actions: 43` hugs the icon button); (d) widths persisted via `_clientsColumnPrefsStorage` (`SecureStorage.setClientsColumnPrefsJson`) — debounced 400 ms + circuit-broken after two consecutive write failures (one cleanup + one retry — the plugin's `load()` deletes a corrupt DPAPI blob on the first decrypt-fail then rethrows, so the retry writes into a fresh store); hydrated from `main.dart` via `hydrateClientsColumnPrefs(SecureStorage)` before `runApp` so the first mount sees the operator's last-saved widths; (e) all text cells use `maxLines: 1 + overflow: ellipsis` and the actions cell is wrapped in `ClipRect + Align(center)` so shrinking the actions column past the icon width clips horizontally instead of wrapping to a second row
    │
    ...
    │       ├── screens/library_screen.dart                 # Create/scan/upload/filter libraries; LibraryScreen({embedded = false}); alphabetical pre-selection; didChangeDependencies → cubit.refresh() (stale-while-revalidate); manual double-tap via _lastTapAt; sandwich gradient on cards; faded type-icon fallback; _SmallStatTile (compact); _TypeFilterChips; _SkeletonBody.  **2026-05-20 polish:** type filter is a `FluxFilterPill<String>` next to a matching `FluxFilterPill<_SortBy>` (the standalone _SortMenu was removed — both controls now share the FluxToolbarPillButton chrome with `bg-raised` rest fill so they read as peers of the view-mode toggle). Add Library dialog rebuilt on `FluxGlassDialog` shared widgets: `_DialogTextField` + `_FieldLabel` for the name/path inputs, FluxFilterPill type picker (Movies/TV/Music/Photos/Documents/Other — the new "Other" entry replaces the "Documents" catch-all for non-media folders), `FluxButton` actions, and a `_RowCloseButton` in the header.  Folder picker calls `pickFolderPath()` (Plex-style — see `shared/util/folder_picker.dart`) which on Windows uses Win32 `IFileSaveDialog` with a phantom filename + `FOS_FORCEFILESYSTEM` so the picker actually shows files inside each folder (the default `IFileDialog` with `FOS_PICKFOLDERS` hides them — operators couldn't see what was in a folder before selecting it); other platforms fall back to `file_selector.getDirectoryPath()`.  `file_picker` removed from `pubspec.yaml`; new deps `file_selector ^1.1.0` + `win32 ^5.15.0` + `ffi ^2.2.0`
