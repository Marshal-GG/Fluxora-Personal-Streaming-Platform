...
└── features/
    ...
    │
    ├── groups/       # ✅ Plan 13 §M4 + §M6 + §M8 mobile (2026-05-07) — Profile-screen group surfaces + PIN modals
    ...
    │   └── presentation/
    │       ├── cubit/groups_cubit.dart       # MobileGroupsCubit — load + refreshSilent + enter + enroll + changePin + lock + lockAll(); singleton-scoped via GetIt so cards survive bottom-tab hops
    │       ├── cubit/groups_state.dart       # MobileGroupsLoaded with lockedGroups + unlockedGroups filtered getters; MobileGroupRow with enrollmentState routing helper
    │       └── widgets/
    │           ├── groups_section.dart       # Three Profile cards (Locked / Unlocked / Visible Libraries); self-hides when nothing to surface; per-row "Lock" button + "Lock all" header action with confirm dialog
