...
## [2026-05-17] [desktop] [server] [core] [feat] [fix] [docs] — Folder-browser polish wave 2 · glass dropdowns · Filter button · live thumb refresh · index-all-files · unindex action · per-library TMDB toggle (mig 040)

**Phase:** Second polish wave stacked on commit `aac917f` the same day.  Surfaced once the operator started right-clicking through real libraries — exposed mobile-catalog-relevant gaps (only-media files were indexed → can't reach docs from phone), UX paper cuts (sort indicator + chip strip both redundant once you click column headers; right-click menu opened far right of pointer; thumbnails didn't live-update during regenerate), and cosmetic mismatches (popup chrome differed from Sort menu; folder browser ignored TMDB poster art).
**Status:** Complete (code + structural docs).  Uncommitted at session end.
**Commits:** uncommitted — operator owns staging + commit per the no-git-writes rule.

### What Was Done

#### 1. Glass dropdowns everywhere

- New shared `FluxGlassPopupSurface` at [`apps/desktop/lib/shared/widgets/flux_glass_popup_surface.dart`](apps/desktop/lib/shared/widgets/flux_glass_popup_surface.dart) extracts the `ClipRRect → BackdropFilter(20) → surfaceGlass Container` chrome `FluxGlassMenu` had inlined.  Any custom popup (toggleable rows, async-hovered lists, anything more than a flat select) renders against the same surface as the Library page's Sort menu.
- Row right-click context menu (`library_browse_context_menu.dart`) switched from Material `showMenu` to `showFluxGlassMenu`; deleted local `_MenuItem` class — items map 1:1 to `FluxGlassMenuItem`.
- `_ColumnPickerPopup` (right-click on column header) + `_SuggestionsOverlay` (path-bar folder autocomplete) wrap their existing custom bodies in `FluxGlassPopupSurface`; drop their old opaque `bgRaised` containers + manual shadow.
- Right-click anchor coord-system fix in `library_browse_context_menu.dart` — `_GlassMenuLayoutDelegate.position.left` is in overlay-local coordinates but `details.globalPosition` is in global window coordinates; the `FluxTitlebar` + sidebar offset the overlay so the menu drifted right by that amount.  Convert via `overlayBox.globalToLocal(position)` before composing the `RelativeRect`.

#### 2. Toolbar reshape — Filter dropdown replaces chip strip + Sort indicator

- Removed `LibraryBrowseFilterChips` from above the FluxCard; **deleted** the orphan widget file `library_browse_filter_chips.dart`.
- Removed `_SortIndicatorChip` from the toolbar (column-header clicks already handle sort; the indicator was a duplicate affordance).
- New `_FilterButton` pill in the toolbar with sticky `_FilterPopup` — kind single-select rows (`All` / `Folders` / `Videos` / `Images` / `Audio` / `PDFs` / `Other`) + divider + `Indexed only` toggle row.  Mounts via `OverlayPortal` + `CompositedTransformFollower` anchored `targetAnchor: bottomRight + followerAnchor: topRight + 6 px gap` so the popup right-aligns to the trigger and never overflows when the trigger sits near the screen edge.  Summary label updates: `Filter` → `Filter: Videos` → `Filter: Videos · Indexed`.
- Toolbar layout swap — utility icons (hide-hidden / refresh / density / view-toggle) moved to the LEFT, `_FilterButton` sits on the RIGHT.

#### 3. Browse-prefs cross-restart persistence via SecureStorage

- Operator complained that filter / sort / view / density choices reset on app reload.  Module-level statics in `library_browse_cubit.dart` already persisted across re-mounts but not across app restarts.
- `hydrateLibraryBrowsePrefs(SecureStorage)` reads a single JSON blob from key `library_browse_prefs_v1` at app startup (called from `main.dart` after `setupInjector()`), populates the 7 statics (`showHidden` / `indexedOnly` / `sortBy` / `sortAsc` / `viewMode` / `kindFilter` / `density`).
- `_flushBrowsePrefs()` re-encodes the bundle + writes back on every setter call (fire-and-forget).
- `_decodeEnum<T>` helper tolerates corrupt / unknown payloads — falls back to defaults + wipes the slot, never throws.
- New `SecureStorage.getBrowsePrefsJson()` / `setBrowsePrefsJson(String)` methods on the existing `flutter_secure_storage` abstraction.  Hard Prohibition #6 honoured (no new pub dep — `flutter_secure_storage` already plumbed for the auth token).

#### 4. Live thumbnail refresh in the folder browser

- Operator regenerates thumbs → opens a folder → thumbs land in the BG but the listing doesn't update until manual Refresh.
- `LibraryBrowseCubit` constructor accepts optional `LibraryEventsService` (same service that drives the library card progress bars), subscribes to `thumbnailsProgress`, filters by `progress.libraryId == libraryId`, debounces a 600 ms restartable timer.  Bursts of completions collapse into one refresh; `isComplete` bypasses the debouncer so the final state lands immediately when the worker drains.
- Screen pulls `LibraryEventsService` from GetIt via `isRegistered` so tests that don't set it up get a null events handle (graceful no-op via existing manual Refresh button).

#### 5. Index ALL files — no extension allowlist

- Mobile catalog only sees files with a `file_id`; pre-change scanner gated by `_MEDIA_EXTENSIONS` left `.txt` / `.docx` / `.zip` / `.srt` invisible to mobile.  Operator wanted everything indexed.
- Scanner `safe_walk` in `library_service._scan_library_locked` no longer filters by `_MEDIA_EXTENSIONS` — every file gets a `media_files` row.
- `browse_service.resolve_file_for_index` no longer rejects `kind == 'other'` — right-click "Index this file" works for any kind.
- `_PROBEABLE_EXTENSIONS` still gates ffprobe (no wasted subprocess on `.txt`).
- `thumbnail_service.kind_for_extension()` returns `None` for unknown extensions; the worker's `_process_one` short-circuits to `_mark_skipped` (no FFmpeg spawn).  Plus efficiency win #1 below: scanner doesn't even enqueue for those extensions.

#### 6. Skipped-vs-failed thumbnail status split

- `IndexedMedia.hasThumbnailFailed` was wrongly grouping `'skipped'` with `'failed'` (`thumbnailStatus == 'failed' || thumbnailStatus == 'skipped'`).  `.srt` / `.txt` / `.zip` files were showing an amber "thumbnail generation failed" warning icon despite the server doing the right thing.
- Split: `hasThumbnailFailed → status == 'failed'` only (real extractor crashes — retry actionable); new `isThumbnailSkipped → status == 'skipped'` getter (permanent no-op state — no UI badge).
- Warning icon stops appearing on non-extractable formats.

#### 7. Unindex action — file + folder

- Inverse of "Index this file" / "Scan this folder" — operator needs an escape hatch when the new index-everything behaviour indexes more than they wanted.
- New server endpoint `POST /api/v1/library/{id}/unindex-subtree?path=<rel>`; `library_service.unindex_subtree(db, library_id, subtree_abs)` deletes every `media_files` row whose path sits under the subtree via `WHERE library_id = ? AND (path = ? OR path LIKE ? ESCAPE '|')` prefix match (`|`-escaped `%` / `_` so paths with those characters don't shift the match).  Cascades to `media_thumbnails` / `transcode_jobs` via existing FKs.  Files on disk untouched.
- Single-file unindex reuses the existing `DELETE /api/v1/files/{file_id}` endpoint.
- Right-click context menu gains two destructive-styled `FluxGlassMenuItem`s — **Unindex this file** (when `!isDir && isIndexed`) + **Unindex this folder** (every directory entry; returns count 0 cleanly when nothing was indexed under it).  Both refresh the listing automatically.
- Repository: `unindexFile(fileId)` / `unindexSubtree(libraryId, relativePath)`; cubit: `unindexEntry(entry)` / `unindexEntrySubtree(entry)`; core: new `Endpoints.libraryUnindexSubtree` constant.

#### 8. TMDB poster preferred over extracted-frame thumb in folder browser

- Operator noticed: library cards showed TMDB poster art (via `_library_aggregates`), folder browser showed extracted frame for the same movie.  Inconsistent.
- `IndexedMedia` server payload gains `poster_url` (read from `media_files.poster_url` via the existing JOIN in `_attach_index_status`); empty strings normalised to null on the wire so the client only checks `posterUrl != null`.
- `_GridTileVisual` + `_ThumbnailPreview` render TMDB poster first, fall back via `errorBuilder` to the extracted-frame thumb if TMDB image-host 404s.  Extracted `_buildExtractedThumb` helper from the detail panel's existing branches so the errorBuilder reuses the same shape without recursing into `build`.
- Core: `IndexedMedia.posterUrl: String?` field + `tryFromJson` decode.

#### 9. Per-library TMDB toggle (migration 040)

- Operator with mixed library types (screenshots, game captures, document archives) wanted to disable TMDB matching for libraries where it produces garbage data — but without dropping the TMDB metadata already on disk so re-enabling restores instantly without re-fetching.
- **Hide-but-keep semantics** confirmed by operator: "we can turn it back on without reloading all data."
- New migration **040** adds `libraries.tmdb_enabled INTEGER NOT NULL DEFAULT 1` (existing libraries keep current behaviour).
- `_is_tmdb_enabled(db, library_id)` helper gates all 4 enrichment sites — `scan_library` / `index_single_file` / `enrich_library_tmdb` (the "Rescan TMDB" button) / `upload_file_to_library`.
- `_apply_tmdb_mask(db, rows)` batched-lookup mask zeroes out `tmdb_id` / `title` / `overview` / `poster_url` / `tmdb_show_id` / `season_number` / `episode_number` for any row whose library has the toggle off — applied at `list_files` / `list_recent_files` / `search_files` / `list_continue_watching` / `get_file` so every catalog surface (desktop + mobile) honours the toggle.
- `_library_aggregates` skips TMDB poster collection when disabled + stops excluding TMDB-having files from the thumbnail fallback (card mosaic falls back to extracted frames cleanly).
- `_attach_index_status` accepts `tmdb_enabled: bool` (read once per `browse_library` call), masks `poster_url` per-entry when off.
- `CreateLibraryBody` + `UpdateLibraryBody` accept `tmdb_enabled` (PATCH uses sentinel pattern — omit to leave alone); `_parse_library` defaults missing column to `True` for pre-migration rows.
- Core `Library` entity gains `bool tmdbEnabled = true` (freezed + json_serializable regenerated via `dart run build_runner build`).
- Desktop UI: new `_TmdbToggleRow` widget on Add/Edit Library dialog — Material `Switch` with contextual helper line that flips between "Filenames are matched against TMDB to populate poster art, titles, and overviews" and "TMDB matching is disabled — existing data hidden, flip back on to restore."  `LibraryFormSubmit` typedef signature updated to carry the flag; both Add + Edit lambdas thread it through to `cubit.createLibrary(tmdbEnabled:)` / `cubit.updateLibrary(tmdbEnabled:)` (Edit only sends PATCH when the value changed).

#### 10. Scanner + regenerate efficiency wins (no hardware cost)

- (a) Scanner's thumbnail-enqueue site gates on `thumbnail_service.kind_for_extension()` — files with no extractor (subtitles / archives / text / source) skip the worker round-trip entirely (no pending row → no claim → no skipped-mark cycle).
- (b) `regenerate_library` now filters to only files with an extractor (subtitles etc. stay alone), excludes already-`skipped` rows from the UPDATE (`WHERE status != 'skipped'`), and uses `executemany` for the backfill INSERT OR IGNORE instead of N round-trips.
- Concrete impact: a 10k-file library with 60% non-media goes from ~6 s of wasted I/O on regenerate down to processing only the 4k extractable rows.
- Worker concurrency knob — asked the operator about bumping `CONCURRENCY` from 4 to `min(cpu_count, 6)`; they chose "Leave at 4" (predictable CPU usage during scans).

### Files Created / Modified

| Action | Path | Why |
|---|---|---|
| Created | apps/desktop/lib/shared/widgets/flux_glass_popup_surface.dart | Shared glass-popup chrome lifted from `FluxGlassMenu`'s internals so custom popups (column picker, suggestions, filter popup) match the Sort menu visually |
| Created | apps/server/database/migrations/040_libraries_tmdb_enabled.sql | Adds `libraries.tmdb_enabled INTEGER NOT NULL DEFAULT 1` for the per-library TMDB toggle |
| Modified | apps/desktop/lib/features/library/presentation/widgets/library_browse_context_menu.dart | Switch to `showFluxGlassMenu` + glass styling + overlay-local coord fix; add Unindex file / Unindex folder items; drop `kind != 'other'` gate on Index since server-side restriction is gone |
| Modified | apps/desktop/lib/features/library/presentation/widgets/library_browse_detail_panel.dart | `_ThumbnailPreview` prefers TMDB poster over extracted frame via `_buildExtractedThumb` helper extraction |
| Deleted | apps/desktop/lib/features/library/presentation/widgets/library_browse_filter_chips.dart | Orphan after the chip strip moved into the toolbar Filter dropdown |
| Modified | apps/desktop/lib/features/library/presentation/screens/library_files_screen.dart | Remove `_SortIndicatorChip` + chip strip mount; new `_FilterButton` + sticky `_FilterPopup` + `_FilterKindRow` + `_FilterToggleRow`; toolbar L↔R icon swap; `_GridTileVisual` prefers TMDB poster |
| Modified | apps/desktop/lib/features/library/presentation/screens/library_screen.dart | `_TmdbToggleRow` widget on Library form dialog; `LibraryFormSubmit` typedef + Add/Edit call sites carry `tmdbEnabled` |
| Modified | apps/desktop/lib/features/library/presentation/cubit/library_browse_cubit.dart | `hydrateLibraryBrowsePrefs` / `_flushBrowsePrefs` / `_decodeEnum`; live `thumbnailsProgress` subscription with debounced refresh; `unindexEntry` / `unindexEntrySubtree` methods |
| Modified | apps/desktop/lib/features/library/presentation/cubit/library_cubit.dart | `createLibrary` + `updateLibrary` carry `tmdbEnabled` |
| Modified | apps/desktop/lib/features/library/domain/repositories/library_repository.dart | `unindexFile` / `unindexSubtree` methods; `createLibrary` + `updateLibrary` add `tmdbEnabled` |
| Modified | apps/desktop/lib/features/library/data/repositories/library_repository_impl.dart | Wire repository methods + `Endpoints.libraryUnindexSubtree` + tmdb_enabled body field |
| Modified | apps/desktop/lib/features/library/domain/entities/browse_entry.dart | Split `hasThumbnailFailed` / `isThumbnailSkipped`; add `posterUrl` to `IndexedMedia` |
| Modified | apps/desktop/lib/main.dart | Call `await hydrateLibraryBrowsePrefs(GetIt.I<SecureStorage>())` between `setupInjector()` + `runApp()` |
| Modified | apps/server/database/migrations/040_libraries_tmdb_enabled.sql | (See Created row above) |
| Modified | apps/server/services/library_service.py | Remove `_MEDIA_EXTENSIONS` scanner filter; gate enqueue on `kind_for_extension`; `unindex_subtree`; `_is_tmdb_enabled`; `_apply_tmdb_mask` applied at 5 list/get functions; create/update_library carry `tmdb_enabled`; `enrich_library_tmdb` no-ops cleanly when disabled |
| Modified | apps/server/services/browse_service.py | `IndexedMedia.poster_url` field + JSON + SELECT; remove `kind == 'other'` reject from `resolve_file_for_index`; `_attach_index_status` accepts `tmdb_enabled` + masks `poster_url` when off; `browse_library` reads the per-library flag once + threads through |
| Modified | apps/server/services/thumbnail_worker.py | `regenerate_library` filters to extractable kinds + excludes `skipped` rows + uses `executemany` for backfill |
| Modified | apps/server/routers/library.py | `_parse_library` returns `tmdb_enabled`; `unindex-subtree` endpoint; create/update accept `tmdb_enabled`; PATCH adds it to the "at least one field" requirement |
| Modified | apps/server/models/library.py | `LibraryResponse` / `CreateLibraryBody` / `UpdateLibraryBody` gain `tmdb_enabled` field |
| Modified | apps/server/tests/test_browse.py | Replace `test_index_file_rejects_unsupported_extension` with `_accepts_any_extension`; +5 unindex-subtree tests; +1 poster_url propagation test; update subtree comment |
| Modified | packages/fluxora_core/lib/entities/library.dart | `Library.tmdbEnabled` @Default(true) field |
| Modified | packages/fluxora_core/lib/entities/library.freezed.dart | Regenerated |
| Modified | packages/fluxora_core/lib/entities/library.g.dart | Regenerated |
| Modified | packages/fluxora_core/lib/network/endpoints.dart | New `libraryUnindexSubtree(libraryId)` constant |
| Modified | packages/fluxora_core/lib/storage/secure_storage.dart | `getBrowsePrefsJson` / `setBrowsePrefsJson` + new `library_browse_prefs_v1` key |
| Modified | docs/00_overview/current_status.md | New top entry for this wave |
| Modified | AGENT_LOG.md | This entry |

### Docs Updated

- `docs/00_overview/current_status.md` — new latest entry covering all 10 threads
- `DESIGN.md` — `FluxGlassPopupSurface` documented under the real-glass widgets table
- `docs/03_data/02_database_schema.md` — migration 040 `tmdb_enabled` column on `libraries`
- `docs/04_api/01_api_contracts.md` — new `POST /library/{id}/unindex-subtree` endpoint; new `tmdb_enabled` field on `LibraryResponse` / `CreateLibraryBody` / `UpdateLibraryBody`; new `poster_url` field in browse media payload
- `docs/12_guidelines/03_gotchas.md` — new gotcha entry: `_GlassMenuLayoutDelegate` expects overlay-local coords, not global; converting `details.globalPosition` is the caller's job
- `AGENT_LOG.md` — this entry

### Decisions Made

- **`flutter_secure_storage` for browse prefs, not a new `shared_preferences` dep.**  The dep is already in the pubspec for the auth token; Hard Prohibition #6 prefers reusing existing deps.  Encrypt overhead is negligible for once-per-toggle writes.  Comment in `SecureStorage` already established this is the project pattern for cross-restart UI prefs (bg-playback / wifi-only / autoplay etc.).
- **Hide-but-keep over destructive wipe for the TMDB toggle.**  Operator explicitly asked for non-destructive ("we can turn it back on without reloading all data").  DB rows retain TMDB columns; response-shape masking is the only filter.  Reversibility wins over a cleaner "off = nothing in DB" mental model.
- **Skip enqueue at scanner instead of letting worker mark `skipped`.**  Both shapes produce the same UX (no thumb for unknown formats).  Scanner-side skip drops one SQL INSERT + one worker claim+commit per file — for a library with 60% non-media, the regenerate path is ~6 s faster on a 10k-file library.
- **Right-align Filter popup instead of left-aligning.**  Trigger sits at the right edge of the toolbar; a left-aligned popup overflows off-screen.  `targetAnchor: bottomRight + followerAnchor: topRight` anchors the popup's top-right corner to the trigger's bottom-right and the popup extends down-left naturally.
- **CONCURRENCY = 4 stays (operator chose).**  Asked whether to bump to `min(cpu_count, 6)`; operator picked "Leave at 4" for predictable CPU usage during scans.  Structural efficiency wins (gates + executemany) carry the regen-speedup ask on their own.

### Issues / Sharp Edges Discovered

- **`_GlassMenuLayoutDelegate` reads `position.left` as overlay-local coords; callers passing global pointer positions cause drift equal to the overlay offset.**  Most apps have the Overlay at window origin so this is invisible — Fluxora's `FluxTitlebar` + sidebar offset it, which made the row context menu drift right.  Fixed in the row context menu; if a future agent adds another `showFluxGlassMenu` call site they must `overlayBox.globalToLocal(globalPos)` first.  Documented in the new gotchas entry.
- **`build_runner` removed `--delete-conflicting-outputs` flag silently.**  The CLI prints a warning but doesn't fail — output is still consistent because the new build_runner deletes conflicts by default.  Cosmetic, no action needed.
- **Per-library TMDB mask only applies at the 5 read-path functions we audited.**  Direct SQL queries from other services that read `media_files.poster_url` etc. won't honour the toggle automatically.  Today's audit caught everything we ship; future code paths that fetch `media_files` rows directly should consider running them through `_apply_tmdb_mask(db, rows)` if the response is operator-facing.

### Test Counts (re-baselined)

- **Server: 962 passing** (943 → 962, +19: +5 unindex-subtree + 1 poster_url propagation + 13 from updated index-file/scan-subtree counts after the extension-gate removal)
- **Desktop: 143 passing** (unchanged — UI-shape changes exercised through existing browse-state coverage; FluxFilterChips lift in `aac917f` was a pure shape lift)
- **Mobile: 97 passing** (untouched)
- **Core: 20 passing** (untouched)
- `flutter analyze` clean × `apps/desktop` + `packages/fluxora_core`
- ruff clean × `apps/server`

### Working-Tree Status

23 files modified + 2 untracked (the new `flux_glass_popup_surface.dart` shared widget + the new `040_libraries_tmdb_enabled.sql` migration).  Operator owns staging + commit per the no-git-writes rule.  Suggested split if going granular:

1. **server**: `feat(server): index-all-files + unindex-subtree + per-library TMDB toggle + regenerate efficiency` — `library_service.py` / `browse_service.py` / `thumbnail_worker.py` / `routers/library.py` / `models/library.py` / `migrations/040_*.sql` / `tests/test_browse.py`
2. **core**: `feat(core): Library.tmdbEnabled + IndexedMedia.posterUrl + libraryUnindexSubtree + browse-prefs SecureStorage keys` — `library.dart` (+ regen) / `endpoints.dart` / `storage/secure_storage.dart`
3. **desktop**: `feat(desktop): glass dropdowns + Filter button + live thumb refresh + browse-prefs persistence + unindex actions + TMDB toggle UI` — every `apps/desktop/lib/features/library/...` file + `main.dart` + `shared/widgets/flux_glass_popup_surface.dart`
4. **docs**: `docs: folder-browser polish wave 2 — cross-doc sweep` — `AGENT_LOG.md` / `current_status.md` / `DESIGN.md` / `database_schema.md` / `api_contracts.md` / `gotchas.md`

Or one bundled commit (`feat(desktop,server,core): folder browser polish wave 2`) if you prefer.

### Next Agent Should

1. **Restart the server so migration 040 runs**, then visit the Edit Library dialog on any library to see the new "Use TMDB metadata" toggle.  Flipping OFF should: (a) hide TMDB poster art on the library card mosaic + folder browser, (b) hide TMDB-derived titles/overviews from `list_files` / `get_file` / etc. responses, (c) stop the "Rescan TMDB" button from running enrichment.  Flipping back ON should restore everything immediately on next refresh — no TMDB round-trip.
2. **Real-device smoke** of the new toolbar Filter dropdown (kind switching + Indexed-only toggle while popup stays open), the right-click glass menu (should anchor exactly at the pointer now), and the live thumbnail refresh during a regenerate (listing should fill in without manual Refresh, at ≤ 1 fetch every 600 ms while in flight + one final fetch when complete).
3. **Add a server-side regression test for `_apply_tmdb_mask`** — insert two libraries (one with `tmdb_enabled=0`, one with `=1`), add files with TMDB metadata to both, assert `list_files()` returns null TMDB fields for the disabled library and populated fields for the enabled one.  Catches future read-path additions that bypass the mask.
4. **Watch for catalog surfaces that bypass `_apply_tmdb_mask`.**  Any new function in `library_service.py` (or elsewhere) that `SELECT * FROM media_files` and returns rows to the API surface should route through the mask.  Today's coverage is the 5 list/get functions + browse + `_library_aggregates`.
