...
CREATE TABLE libraries (
    ...
    av1_stream_copy_override INTEGER,                 -- migration 030: tri-state per-library AV1 passthrough (NULL=inherit, 0=off, 1=on)
    vp9_stream_copy_override INTEGER,                 -- migration 030: tri-state per-library VP9 passthrough
    tmdb_enabled INTEGER NOT NULL DEFAULT 1           -- migration 040: per-library TMDB toggle (0 hides + skips enrichment, 1 enabled)
);
...
| `034_client_audio_codec_blocklist.sql` | Plan 21 — mirrors `033` for audio codec fallback.  See [`docs/10_planning/archive/21_client_audio_decoding.md`](../10_planning/archive/21_client_audio_decoding.md) for the field-level breakdown. |
| `035_media_files_audio_tracks.sql` | Plan 22 — adds `audio_tracks TEXT` JSON to `media_files` for multi-audio-track support.  See [`docs/10_planning/archive/22_multi_audio_track_support.md`](../10_planning/archive/22_multi_audio_track_support.md). |
| `036_stream_sessions_fk_set_null.sql` | Relaxes the `stream_sessions.file_id` FK from `NOT NULL REFERENCES` to `REFERENCES ... ON DELETE SET NULL` so unindex / delete-file cascade no longer trips on prior streaming sessions. |
| `037_media_thumbnails.sql` | Plan 27 M1 — `media_thumbnails` queue table (`file_id PK ON DELETE CASCADE` to `media_files`, `status` / `priority` / `attempts` / `error_message` / `generated_at` / `created_at` / `updated_at`).  Drives the BG thumbnail worker.  Index on `(priority DESC, created_at ASC)` powers the worker's `UPDATE ... RETURNING` atomic claim. |
| `038_notifications_thumbnail_category.sql` | Plan 27 M2 — widens `notifications.category` CHECK to accept `'thumbnail'` for aggregated failure notifications. |
| `039_user_settings_thumbnail_width.sql` | Plan 27 M4 — `user_settings.thumbnail_width INTEGER NOT NULL DEFAULT 320`.  Operator-configurable (160-640 enforced at the Pydantic settings router boundary).  Worker reads at start of each claim cycle. |
| `040_libraries_tmdb_enabled.sql` | Per-library TMDB-enrichment toggle.  Adds `tmdb_enabled INTEGER NOT NULL DEFAULT 1` to `libraries`.  Default ON so existing libraries continue enriching.  Hide-but-keep semantics: when 0, the server (a) skips all future TMDB enrichment passes for files in this library, and (b) masks already-persisted TMDB columns (`tmdb_id` / `title` / `overview` / `poster_url` / `tmdb_show_id` / `season_number` / `episode_number`) out of every read-path response (`list_files` / `list_recent_files` / `search_files` / `list_continue_watching` / `get_file` / `_library_aggregates` / `_attach_index_status`) without dropping the rows from disk.  Flipping back to 1 restores everything instantly — no TMDB round-trip needed.  Operator toggles via Add/Edit Library dialog → "Use TMDB metadata" switch.  Pre-migration rows materialise the column with default 1 so behaviour stays unchanged on upgrade. |
