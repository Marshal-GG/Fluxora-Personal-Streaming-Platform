...
| Media3 `TonemappingRenderersFactory` (Android native, hand-rolled) | — | Custom `DefaultRenderersFactory` subclass at `apps/mobile/android/app/src/main/kotlin/dev/marshalx/fluxora_mobile/exo/TonemappingRenderersFactory.kt`. Overrides `buildVideoRenderers` to install `TonemappingVideoRenderer` (a `MediaCodecVideoRenderer` subclass) that injects `mediaFormat.setInteger("color-transfer-request", 3)` (= `KEY_COLOR_TRANSFER_REQUEST` + `COLOR_TRANSFER_SDR_VIDEO`) on API 33+ so the hardware codec tone-maps HDR10 / HLG / DolbyVision → SDR BT.709 before frames reach the SDR `SurfaceProducer` texture. Wired into the player via `ExoPlayer.Builder(context).setRenderersFactory(TonemappingRenderersFactory(...))` in `FluxoraExoPlayer.kt`. No-op on Android <13 or codecs that ignore the request — operators can still fall back to server-side `?tonemap=true` via the player overflow toggle. |
| `junit:junit` (Android native, test) | 4.13.2 | JUnit suite for the Kotlin pure-function helpers (`buildAudioTrackMapping`, `parseSourceIndex`, `mapPlayerErrorCode`, `firstSelectedVideoFormat`) in `apps/mobile/android/app/src/test/`. |
