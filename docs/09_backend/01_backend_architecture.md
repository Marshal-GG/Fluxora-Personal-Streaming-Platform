...
│   ├── auth_service.py     # HMAC-SHA256 tokens, pairing state machine; `revoke_client(db, client_id)` shared between operator-driven `DELETE /auth/revoke/{id}` and mobile self-revoke `DELETE /clients/me`; `delete_client(db, client_id)` parameterised `DELETE FROM clients` for the operator-driven hard-delete (`DELETE /auth/clients/{id}` — FK cascades handle dependent rows); `update_client_display_name(db, client_id, display_name)` parameterised UPDATE — backs `PATCH /clients/me` (settings remediation M2.5, 2026-05-08) ✅
...
| `auth_service` ✅ | Token generation (HMAC-SHA256), pairing state machine, token validation, **per-request heartbeat** (migration 023). `list_clients` rewritten to LEFT-JOIN `stream_sessions WHERE ended_at IS NULL` via `ROW_NUMBER() OVER (PARTITION BY client_id ORDER BY started_at DESC) = 1` so each client carries one in-flight session at most. `create_pair_request` accepts `client_ip` + persists to `clients.last_ip` with `COALESCE` so a None caller doesn't clobber a previous value. New `update_client_heartbeat(db, client_id, last_ip=None)` helper is the single write path for `last_seen` + optional `last_ip`; called from `validate_token` (best-effort — try/except + WARNING log so a transient SQLite write failure can't 401 a valid request). **Side effect**: before migration 023 `last_seen` was frozen at pair / approval; it is now genuinely live.  **Settings remediation M2.5 (2026-05-08):** new `update_client_display_name(db, client_id, display_name)` parameterised `UPDATE clients SET name = ?, last_seen = ? WHERE id = ?` backs the new `PATCH /auth/clients/me` self-rename route — body validated by the `UpdateClientMeRequest` Pydantic model (1–50 chars, trims whitespace, rejects blank-after-trim + control chars `\x00-\x1f`).  **2026-05-20:** new `delete_client(db, client_id)` parameterised `DELETE FROM clients WHERE id = ?` backs the new `DELETE /auth/clients/{id}` operator hard-delete route — physically removes the row (distinct from `revoke_client` which only flips status). `ON DELETE CASCADE` on `group_members` and `ON DELETE SET NULL` on `stream_sessions.client_id` clean up dependent rows automatically. Surfaced from the desktop Clients screen's Revoked-filter view so the operator can prune devices that are revoked but still cluttering the table. | `create_pair_request(client_id, device_name, platform, app_version, email, client_ip)`, `approve_client()`, `reject_client()`, `revoke_client()`, `delete_client(client_id)`, `update_client_display_name(client_id, display_name)`, `get_trusted_client_by_token()`, `update_client_heartbeat(client_id, last_ip)`, `list_clients()` |
...
**Plan 21 (2026-05-12):** audio is also stream-copied when the source audio codec is in `_AUDIO_STREAM_COPY_ALLOWLIST = frozenset({"aac", "ac3", "eac3", "opus", "flac"})`, HDR tonemap is inactive, and the client does not have a `client_audio_codec_blocklist` row for that audio codec. When audio stream-copy is used with a non-AAC codec (AC3, EAC3, Opus, FLAC), the segment type is forced to **fmp4** because MPEG-TS does not cleanly carry those codecs across all HLS clients; fmp4 (HLSv7+) handles all of them. The `_resolve_audio_passthrough(session_id, source_audio_codec, apply_hdr_tonemap, source_audio_tracks=None)` helper encodes this decision.

**Plan 22 (2026-05-14):** multi-audio-track support. `-map` flags are now conditional: `audio_passthrough=True` emits `-map 0:v:0 -map 0:a?` (every audio track in the source); `audio_passthrough=False` emits `-map 0:v:0 -map 0:a:0?` (single first-track for re-encode paths). `_ensure_fmp4_init_segment` updated to declare every audio track in its moov so the init segment matches segments delivered by the main pipeline. New `_probe_audio_tracks(file_path)` helper returns per-track metadata `[{index, codec, language, title, channels, sample_rate, bit_rate}, ...]`; results cache in `_session_audio_tracks[session_id]` for the router to read in `StreamStartResponse.audio_tracks` without re-probing. The 4th condition added to `_resolve_audio_passthrough` via the optional `source_audio_tracks` param: when any track has a non-allowlist codec (e.g. AAC + DTS mixed in one Bluray rip), the resolver forces single-track re-encode and the `stream_decision` log emits `audio_reason=audio-mixed-codec-fallback`. Mid-stream track switching is purely client-side on mobile via media_kit's `Player.setAudioTrack` — no server roundtrip.
