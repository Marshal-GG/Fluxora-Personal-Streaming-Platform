...
│   ├── auth.py             # /auth/* ✅ (request-pair, status, approve, reject, revoke; `GET /auth/clients/{id}/visible-libraries` localhost-only — operator "View as" debug returning `VisibleLibraries` snapshot, M5 of `14_groups_management_page.md`)
...
│   ├── stream.py           # GET /sessions, POST /start/{id}?tonemap=, PATCH /{id}/progress, GET/{id}, DELETE/{id} + hls_router; stream-gate hook calls group_service.reason_to_deny_stream (v2 — replaces v1 reason_to_deny); HLS router 2 s segment-wait for seek-ahead (tightened from 5 s on 2026-05-08 — seek-restart shrinks the realistic gap to sub-second; plan §4.3); .m4s/.mp4 served as video/mp4 ✅
...
│       ├── test_files.py        # 21 tests — file listing + filtering + recent + search + reset-progress (validate_token_or_local; +4 reset-progress cases 2026-05-08: zero-out happy / unknown 404 / localhost skips visibility / 404 when library not visible) ✅
...
│       ├── test_groups.py       # 78 tests — CRUD + member management + auth split + stream-gate enforcement (v1 plan); v2 visibility matrix (Public-only / multi-group union / inactive / time-window / member override) + PIN flow service (happy path / wrong PIN / rate limit / strength rejections / revoke / housekeep) + auto-add-to-Public + PIN HTTP routes (/enter / /grant / /grant-status / /master-override) + 14 M8 cases (enroll happy + reject-shared-mode + double-enroll → 409 + non-member rejected + per-client uses member hash / not group hash + enter-without-enrollment → enrollment_required + change happy + wrong-old charges attempt + clear forces re-enroll + visibility branches per-client→enrollment_required_groups + mode-switch shared→per-client + per-client→shared rejects without pin + with pin succeeds + master override works for per-client) + 5 M3 cases for `?include=pin_state` (default shape unchanged + per-client branches + shared mode skips enrollment + 404 on unknown group + recent-failed-attempts window) + 7 M5 cases (view-as localhost-only + view-as 404 unknown client + view-as known-client provenance shape + member time_window_override set+clear + 404 on non-member + member-PATCH localhost-only + member-PATCH clear sentinel) ✅
...
Total: 351 tests passing ✅ (336 through GPU UX Slice C + 15 Phase 6 follow-up cases [widened `_CUVID_FAILURE_MARKERS`, `_resolve_source_metadata` hdr_format path, `_HDR_TO_SDR_VF` chain, `_ensure_fmp4_init_segment`, `_write_static_vod_playlist`, `?tonemap=true` endpoint, `StreamStartResponse` new fields])
```
