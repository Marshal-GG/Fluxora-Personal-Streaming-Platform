...
│   ├── stream.py           # GET /sessions, POST /start/{id}?tonemap=, PATCH /{id}/progress, GET/{id}, DELETE/{id} + hls_router; stream-gate hook calls group_service; HLS router 5 s segment-wait for seek-ahead; .m4s/.mp4 served as video/mp4 ✅
