...
│   ├── auth.py             # /auth/* ✅ (request-pair, status, approve, reject, operator-revoke; `DELETE /auth/clients/me` self-revoke for mobile sign-out — audit §17.3 #3; `GET /auth/clients/{id}/visible-libraries` localhost-only — operator "View as" debug returning `VisibleLibraries` snapshot, M5 of `14_groups_management_page.md`)
