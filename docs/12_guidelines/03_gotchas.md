...
| Per-drag `flutter_secure_storage.write` floods the log on a corrupt DPAPI blob | Caught 2026-05-20 while wiring cross-restart persistence on the Clients-screen column widths.  The on-disk `flutter_secure_storage.dat` is a single DPAPI-encrypted blob; on Windows the plugin's `write()` calls `load()` (decrypt + parse the existing map) before merging the new key.  If the master DPAPI key has changed since the file was written (Windows password reset, profile rebuild, account migration), the decrypt fails — the plugin then tries `file.delete()` so the next write starts from an empty store.  But a column-resize drag fires 50+ writes per second; each one races `load()` against the others' file handles, so `delete()` keeps tripping `OS Error 32 (file is being used by another process)` and the same identical failure scrolls past in the log thousands of times before the drag ends.  Worse, even after fixing the storm, the **first** write of every session is *expected* to fail because that's how the plugin self-heals — naive circuit-breakers that trip on the first failure also lose persistence. | Two coordinated changes: (1) **debounce** the flush.  A column drag is logically one operation, not 50 — `Timer(const Duration(milliseconds: 400), ...)` after the last drag tick collapses the storm to a single write.  (2) **Retry once before tripping a per-session circuit breaker**.  The plugin's self-heal pattern is "decrypt fails → delete file → rethrow"; the retry then writes into an empty store and succeeds.  Only set `_…PrefsWritable = false` if both attempts fail — that distinguishes a healed-on-retry session (good) from genuinely wedged storage like a locked file or missing master key (bad → silence the rest of the session).  Pattern lives in `apps/desktop/lib/features/clients/presentation/screens/clients_screen.dart::_flushClientsColumnPrefs` and `apps/desktop/lib/features/library/presentation/cubit/library_browse_cubit.dart::_flushBrowsePrefs`.  Same shape applies to any future per-pointer-tick secure-storage write. |
