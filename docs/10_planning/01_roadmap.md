...
| Public routing (`fluxora-api.marshalx.dev`) | Should | 🔵 In Progress | Phases 1–4 complete 2026-05-01 (tunnel live; server CF middlewares + admin hardening + `/healthz` + `remote_url` on `/info`; `fluxora_core` dual-base `ApiClient` with `NetworkPathDetector`; mobile pairing flow persists `remote_url` post-pair). Phase 5 desktop wizard + mobile Settings UI pending. v2 multi-tenant plan locked. See [`05_infrastructure/03_public_routing.md`](../05_infrastructure/03_public_routing.md); ADR-013 |
