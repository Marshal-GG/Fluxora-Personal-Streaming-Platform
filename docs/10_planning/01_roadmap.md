...
> **Category:** Planning  
> **Status:** Active - Updated 2026-05-04 (Phase 5 in progress; **server at 297 tests** [Phase A + Phase B backfill: migration 016, FFprobe persistence, `/files/recent`, `/auth/clients/me`, `/files/search`, `/me/continue-watching`, `/me/stats`, same-`client_id` re-pair fix; GPU UX Slice A: encoder-advisor pure-function + `/transcoding/advisor` endpoint, `EncoderTestResult` dataclass, `test_encoder` tuple return, `_input_decoder_args` cuvid hint, cuvid auto-fallback retry, long-GOP stream-copy fix, migration 019 license-key sanitiser]; **mobile at 41 tests** [Phase A + B real-data wiring: `RecentCubit` + `DetailCubit` + `ProfileCubit` + `ContinueWatchingCubit` + `SearchCubit` + `ProfileStatsCubit`; pairing UX rebuild with state-machine + email field + `/reconnect` lost-token recovery + auto-redirect on 401; QR-pairing scanner via `mobile_scanner ^7.1.2`; player polish round — Android PIP + `audio_service ^0.18.18` lockscreen / notification / Bluetooth-headset transport + first-time bg-playback prompt + Profile inline toggle; Downloads tab hidden in v1; mDNS reusePort fix]; **desktop at 63 tests** [GPU UX Slice A: encoder-status widgets + `TranscodingCubit` advisor poll + `ApiException` detail parsing + `ActivityCubit` polling; pair-device dialog with QR via `qr_flutter ^4.1.0`; visible Approve / Reject / Revoke client actions; notifications-audit polish round]; desktop redesign M1–M10 shipped — including M10 custom window chrome / FluxTitlebar / Aero Peek shell-integration; **mobile redesign M0–M9 landed** — Discover surfaces / Detail / Episodes / Player chrome + gestures + sheets / Mini-player + drag-down minimize / Downloads + Profile + Notifications real-data wiring / V2 theme cutover. Next mobile milestone: M10 X-Ray + Group Watch + Offline state shells; iOS PIP + iOS lockscreen pending an iOS test device.)
...
| Hardware encoding (NVENC/VAAPI) | Nice-to-have | 🔵 In Progress | Encoder registry covers 10 encoders (NVENC/QSV/VAAPI/VideoToolbox/software); startup self-test + settings-change retest; GPU monitoring probes; **Slice A shipped 2026-05-04** (encoder advisor + `/transcoding/advisor` + desktop `ActiveEncoderStrip`/`EncoderRecommendationBanner`/`EncoderStatusPanel`; cuvid input-decoder hint + auto-fallback retry; long-GOP stream-copy fix); Slice B (GPU detection) + Slice C (multi-encoder fallback chain) pending owner approval |
