...
| Client-side codec passthrough (plan 19 §M7) | Must | ✅ Done 2026-05-09 (launch priority) | Global `streaming_mode` toggle (`client-decode` default / `server-transcode` legacy fallback) on `user_settings`, migration 028. Server stream-copies AV1 + VP9 sources via fmp4 same as HEVC under client-decode; modern devices hardware-decode at near-zero power cost. Operator toggles via Settings → Encoder Settings → Streaming Mode (`_StreamingModeCard`). Plan: [`docs/10_planning/19_library_transcode_followups.md`](./19_library_transcode_followups.md). M1-M6 + M8 (preset chooser, storage chooser, storage strip, folder tree, edge-case hardening, per-library overrides) deferred to v1.1. |
