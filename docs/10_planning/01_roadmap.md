...
| Per-file thumbnail generation (plan 27) | Must | 📝 Active 2026-05-16 — background asyncio worker generates per-file thumbnails (video frame / image preview / audio embedded art / PDF first page) when TMDB enrichment doesn't apply.  Scan path unchanged — work is queued + processed asynchronously with concurrency cap (default 2) and per-library priority boost (`GET /files?library_id=X` jumps that library to the front).  Six milestones, ~8–10 h: M1 schema (`media_thumbnails` table, migration 037) + extractors (FFmpeg for video/image/audio incl. HDR→SDR Hable tonemap, PyMuPDF for PDF first page); M2 worker w/ atomic `UPDATE...RETURNING` claim + scan-path enqueue + failure-aggregation notification when ≥ 5 files fail per library + orphan-JPEG sweeper; M3 `GET /files/{id}/thumbnail?v=<gen_unix>` endpoint + `cover_urls` aggregation (TMDB first, then thumbs with cache-buster suffix) + `POST /library/{id}/regenerate-thumbnails`; M4 `thumbnail_width` settings field (range 160–640, default 320, migration 038); M5 desktop "Regenerate thumbnails" action tile + Settings → Advanced width slider; M6 sweeper verification + doc sweep.  **New dep:** `pymupdf ^1.24.x` (pure-Python wheel, AGPL-compatible with self-hosted distribution model).  **On-demand endpoint generation explicitly deferred** — BG worker is the only path (per owner direction).  Plan: [`docs/10_planning/27_thumbnail_generation_plan.md`](./27_thumbnail_generation_plan.md). |
