...
| Mobile redesign — M0–M14 fully closed 2026-05-14 | Should | ✅ Done 2026-05-14 — All milestones closed | Plan in [`docs/11_design/mobile_redesign_plan.md`](../11_design/mobile_redesign_plan.md). M0 foundation → M1 shared widgets lift → M2 tab shell + go_router → M3 Discover → M4 Detail + Episodes → M5 Player chrome → M6 Player gestures + 5 bottom sheets → M7 Mini-player + drag-down minimize → M8 Downloads + Profile + Notifications real-data wiring → M9 theme cutover → **M10 Offline + X-Ray + Group Watch UI shells (2026-05-08)** → **M11 Beyond-video viewers (2026-05-08)** → **M12 Onboarding revamp (2026-05-08)** → **M14 Polish + a11y + golden tests (2026-05-14)**. M13 (host-a-server shell) gated on Phase 5+ runtime — not part of redesign cutover. **M14 shipped:** animations to spec (scrim fade 250 ms, transport press 50 ms, ripple 400 ms, tab scale 220 ms, route fade 250 ms), 29 Semantics nodes in player chrome + ~30–35 across 16 screens, `FocusTraversalGroup` on player chrome + form flows, text-scale clamp 1.3× at app root, `golden_toolkit: ^0.15.0`, 5 private widgets exposed public (`PlayerTopBar`, `PlayerCenterTransport`, `PlayerProgressBar`, `PlayerQuickActions`, `PlayerSideRail`), 10 golden baselines. **Test counts: 92 mobile (82 + 10 goldens) / 814 server / 113 desktop / 8 core.** Mobile audit §17 remaining opens: #1 (iOS PIP — blocked on iOS test device) + #9-End-of-episode (needs next-episode resolver). |
...
| Library file-browser power features (plan 28) | Should | 🔵 In Progress 2026-05-16 — **Phase A + B ✅ shipped** (commits `65a3555` server + `f0971ae` client + `6fdfde4` Phase B client).  **Phase A:** Server-side `_attach_index_status` rewritten as one LEFT-JOIN exposing per-indexed-entry `media` payload (width/height/duration_sec/codec_name/hdr_format/audio_codec/thumbnail_status/thumbnail_generated_at_unix/indexed_at_iso/is_streaming) + `mtime_unix` + stale-thumbnail auto-re-queue.  Client gains 320 px right detail panel, single-click-selects + double-click-opens, sortable column headers, list ↔ grid view with real thumbnails for indexed media, in-place search box, indexed-only toolbar toggle, HDR + ▶-live row pills, distinct empty-state copy.  Three new widget files (`LibraryBrowseDetailPanel`, `LibraryBrowseSearchBar`, `LibraryBrowseViewToggle`).  Server suite 916 → 925 (+9).  **Phase B:** type-filter chip row (`LibraryBrowseFilterChips`: All / Folders / Videos / Images / Audio / PDFs / Other single-select kind chips + independent "Indexed only" toggle); item-count footer (`LibraryBrowseCountFooter`, `kHeight=28`) at the bottom showing post-filter `N items · M folders, K files`; keyboard navigation on the listing — ↓↑ step ±1, PageDown/Up ±10, Home/End first/last, Enter opens selection (file or directory), Backspace go-up, Escape clears selection, `/` focuses the search bar (via a new optional `focusNode` param on `LibraryBrowseSearchBar`).  Cubit gains `BrowseKindFilter` enum + `setKindFilter` + `stepSelection(delta)` / `selectFirst` / `selectLast` / `selectOnly` / `clearSelection` / `resolveSelected` helpers.  Polish: failed-thumb warning icon overlay (`Icons.warning_amber_rounded` in `AppColors.amber`) on rows + grid tiles when `media.thumbnail_status == 'failed'`; new `_IndexedTag` widget with `indexed_at` timestamp tooltip (replaces the Phase A bare-text "Indexed" pill).  Long-hover quick-preview popover **deliberately skipped** — right detail panel already shows the same info on click.  Desktop suite **121** (widget tests for chips + footer + key handler land with Phase C).  Phases C (right-click context menu + editable path + per-file Index/Re-thumb endpoints + density + multi-select Ctrl/Shift) / D (back/forward history + lazy folder-size) pending.  Plan: [`docs/10_planning/28_library_file_browser_power_features.md`](./28_library_file_browser_power_features.md). |
| Per-file thumbnail generation (plan 27) | Must | ✅ Done 2026-05-16 — all six milestones shipped same day (commits `217ff8b` → `137362c` → `3dbaf16` → `b0428b0` → `3e8066e` + M6 doc sweep).  Server 814 → 898 (+84); desktop 118 → 121 (+3).  **Post-ship same day:** speed bump (CONCURRENCY 2→4, hwaccel auto + sw fallback, bilinear, q:v 8); real-time per-library progress bars on library cards via new `thumbnails_progress` WS event; `LibraryRepositoryImpl._resolveCoverUrls` fix for server-relative cover_urls; folder-browser MVP (`GET /library/{id}/browse` + `BrowseService` + `library_files_screen.dart` rewrite).  Server **898 → 916** (+18: 5 worker progress-emit + 13 browse endpoint).  Background asyncio worker generates per-file thumbnails (video frame / image preview / audio embedded art / PDF first page) when TMDB enrichment doesn't apply.  Scan path unchanged — work is queued + processed asynchronously with concurrency cap (default 2) and per-library priority boost (`GET /files?library_id=X` jumps that library to the front).  Six milestones, ~8–10 h: M1 schema (`media_thumbnails` table, migration 037) + extractors (FFmpeg for video/image/audio incl. HDR→SDR Hable tonemap, PyMuPDF for PDF first page); M2 worker w/ atomic `UPDATE...RETURNING` claim + scan-path enqueue + failure-aggregation notification when ≥ 5 files fail per library + orphan-JPEG sweeper; M3 `GET /files/{id}/thumbnail?v=<gen_unix>` endpoint + `cover_urls` aggregation (TMDB first, then thumbs with cache-buster suffix) + `POST /library/{id}/regenerate-thumbnails`; M4 `thumbnail_width` settings field (range 160–640, default 320, migration 038); M5 desktop "Regenerate thumbnails" action tile + Settings → Advanced width slider; M6 sweeper verification + doc sweep.  **New dep:** `pymupdf ^1.24.x` (pure-Python wheel, AGPL-compatible with self-hosted distribution model).  **On-demand endpoint generation explicitly deferred** — BG worker is the only path (per owner direction).  Plan: [`docs/10_planning/27_thumbnail_generation_plan.md`](./27_thumbnail_generation_plan.md). |
| Streaming pipeline polish (seek-restart, tonemap timeout, zombie cleanup) | Must | ✅ Done 2026-05-05 + §4 leftovers closed 2026-05-08 + §16 + §17 streaming-pipeline batch closed 2026-05-08 + §17 second-round follow-on closed 2026-05-09 | Four-commit plan in [`11_streaming_pipeline_issues.md`](./11_streaming_pipeline_issues.md) all shipped 2026-05-05 (HDR toggle unblocked, seek-restart wired end-to-end, dedup + zombie cleanup, log rotation pinned).  §4 investigation leftovers closed 2026-05-08: §4.3 segment-wait 5 s → 2 s; §4.5 VOD over-promise tail fixed via post-natural-exit `_finalize_vod_playlist` watcher (replaces spawn-time over-promised playlist with FFmpeg's accurate one on returncode==0); §4.10 "Start over" affordance shipped; §4.8 closed as no-op; §4.7 explicitly deferred (low priority).  **§16 + §17 + same-day follow-on shipped 2026-05-08** ([`16_streaming_resume_and_throttle_plan.md`](./16_streaming_resume_and_throttle_plan.md) + [`17_ffmpeg_diagnostics_and_m2_retry_plan.md`](./17_ffmpeg_diagnostics_and_m2_retry_plan.md)): server-side resume seek end-to-end with mobile `playlistOffsetSec` plumbing for source-time scrubber, audio re-encode under tonemap (PTS alignment), three-path audio branch (copy/re-encode-no-resample/re-encode-resample), uniform `-loglevel info`, new `services/ffmpeg_capabilities.py` version-probe module, transcode-only `-readrate 1.5` + capability-gated `-readrate_initial_burst 30`, 30 s timeout floor when readrate active, mobile `_ProgressBar` Stateful drag-preview state, `POST /stream/start` accepts `?seek_sec=`, `POST /stream/{id}/seek` returns 200 with `applied_seek_sec`.  **§17 second-round follow-on closed 2026-05-09** (plan 17 §10): seekTo backward-out-of-playlist now routes to server-restart (was silently clamping to playlist start); scrubber pin (`_pendingValue` + 5 s fallback timer) holds across the post-emit position-stream-lag transient that was painting the thumb at end-of-track for one frame after every forward server-restart; cubit `seekTo` flips `isSeeking=true` eagerly so the pin's gate is 1:1 with the seeking flag. Sliding-window encoder explicitly deferred to v1.1 with telemetry-driven revisit triggers. |
| Auto streaming mode + opt-in client-error fallback (plan 20) | Must | ✅ Done 2026-05-12 — `'auto'` third streaming mode; per-client codec blocklist; `POST /fallback-transcode` endpoint; `stream_decision` diagnostic log. Plan archived 2026-05-12: [`docs/10_planning/archive/20_auto_streaming_mode.md`](./archive/20_auto_streaming_mode.md). |
| Client-side audio decoding (plan 21) | Should | ✅ Done 2026-05-12 — audio stream-copy for `{aac, ac3, eac3, opus, flac}`; fmp4 switch for non-AAC audio stream-copy; `client_audio_codec_blocklist` (migration 034); `POST /fallback-audio-transcode` endpoint; `audio_streaming_mode` response field; 128k → 256k AAC bitrate bump + `-ac` channel preservation; mobile audio-fallback watcher. Plan archived 2026-05-12: [`docs/10_planning/archive/21_client_audio_decoding.md`](./archive/21_client_audio_decoding.md). |
| Multi-audio-track support (plan 22) | Should | ✅ Done 2026-05-14 — `-map 0:v:0 -map 0:a?` under stream-copy (all tracks); single-track pin retained on re-encode paths; `_ensure_fmp4_init_segment` declares every track; new `_probe_audio_tracks` helper + `_session_audio_tracks` cache; mixed-codec fallback in resolver with `audio-mixed-codec-fallback` reason; migration 035 (`media_files.audio_tracks` JSON); `AudioTrackInfo[]` on `StreamStartResponse`; mobile Audio sheet picker with media_kit client-side track switching. Plan archived 2026-05-14: [`docs/10_planning/archive/22_multi_audio_track_support.md`](./archive/22_multi_audio_track_support.md). |
| Server-restart audio track switching (plan 23) | Must | ✅ Done 2026-05-15 — `POST /api/v1/stream/{session_id}/audio-track` respawns FFmpeg with `-map 0:a:<index>?` + unlinks stale init.mp4 + returns segment-snapped `applied_seek_sec`; cubit re-opens playlist on the player; libmpv `ao=audiotrack` AO override + screen-lifetime `WakelockPlus` fix for Android audio reliability; player UI redesign moves Lock + Fit/Fill icons to top bar and consolidates Audio/Subs/Speed/Quality/Sleep/Cast into the 3-dot overflow menu. **Effectively superseded by plan 24** — endpoint stays as future-client fallback. Plan archived 2026-05-15: [`docs/10_planning/archive/23_audio_track_switching.md`](./archive/23_audio_track_switching.md). |
| Android ExoPlayer migration (plan 24) | Must | 🔵 Active 2026-05-15 — **M1-M4 + M7 + M8 + M9-partial shipped same day (commits `62b2b21` spike, `bdffeb9` abstraction, `5db7e54` Dart engine, `575787e` Kotlin module, `0debbe9` Media3 MediaSessionService, `4ba2a87` seek/scrubber integration, `a1a9c12` plan-tag strip pass, `24d3579` title plumbing + subtitle gate + spike removal + flag cleanup; follow-on commits 2026-05-15 TBD).** Replaces libmpv-on-Android (the source of repeat audio bugs — channel-mask emulation, AudioTrack churn, mid-stream `setAudioTrack` hang) with Media3 ExoPlayer 1.10.1 via a hand-rolled Kotlin platform channel (`MethodChannel('dev.marshalx.fluxora/exo_player')` + `EventChannel('dev.marshalx.fluxora/exo_player_events')`, both keyed by `playerId`). `PlayerEngine` Dart abstraction in `packages/fluxora_core/lib/player/`: `MediaKitEngine` (desktop + iOS unchanged) + `ExoPlayerEngine` (Android default as of M3+M4). Android consumers construct `ExoPlayerEngine` unconditionally post-M9-partial (the `_kEnableExoPlayerEngine` flag was deleted; behaviour is the only path); rollback via `_kForceMediaKitOnAndroid` flag kept until M5+M6 device-green. M7 native Media3 `MediaSessionService` owns lockscreen / notification card / BT transport for the ExoPlayer path; `FluxoraAudioHandler` stays bound for desktop + iOS + Android-rollback. M8 verified seek-restart + scrubber pin + progress reporter under ExoPlayer; closed two engine-cadence mismatches (`open()` state-reset emission + scrubber pin gate). **M9 partial follow-ons (post-`24d3579` real-device verification):** nine fixes landed on operator phone smoke — **LAN cleartext** via `network_security_config.xml` (Media3's `DefaultHttpDataSource` honours Android 9+ default-secure cleartext policy; libmpv didn't) with global `cleartextTrafficPermitted=true` + per-domain HTTPS-only carve-out for `fluxora-api.marshalx.dev`; **client-side HDR→SDR tonemap** via custom `TonemappingRenderersFactory` (subclasses `DefaultRenderersFactory`, injects `MediaFormat.KEY_COLOR_TRANSFER_REQUEST = COLOR_TRANSFER_SDR_VIDEO` on API 33+; server-side `?tonemap=true` remains the fallback on Android <13 / codecs that ignore the request); `PlayerEngine.videoSize` + `videoSizeStream` (Kotlin emits from `onVideoSizeChanged` + `onTracksChanged` via a `firstSelectedVideoFormat` helper; PAR baked into the emitted width for anamorphic sources); three-way `FitMode {fit, fill, stretch}` cycling on the top-bar Fit button (default flipped to `fit`); pinch zoom via raw `Listener` (Flutter's gesture arena lost 2-finger scale to drag at slop crossover; raw pointer-distance math + `_activePointers >= 2` gating fixed it); drag-HUD signals for zoom + fitMode (`"1.25×"` / `"Fit"` / `"Fill"` / `"Stretch"` labels); player UI relayout (transport below the scrubber, `_MinimizeHandle` removed, gradient backdrops on top + bottom chrome); overflow-menu scroll fix (`SingleChildScrollView` + `isScrollControlled: true`). Test counts unchanged from `24d3579`; APK build + `flutter analyze` are the verification artefacts. **M5 multi-audio real-device smoke + M6 HDR+tonemap require operator device testing** (no agent work). **M9 finishes** with `_kForceMediaKitOnAndroid` deletion + plan archival once M5+M6 confirm no regressions (title plumbing + subtitle gate + spike removal + `_kEnableExoPlayerEngine` deletion + gotchas entry + follow-on fixes already in tree). iOS deferred; multi-rendition HLS server output (industry-standard `#EXT-X-MEDIA TYPE=AUDIO` groups) split into plan 25. Plan: [`docs/10_planning/24_player_audio_reliability_plan.md`](./24_player_audio_reliability_plan.md). |
| Client-side codec passthrough + transcode polish (plan 19, all 8 milestones) | Must | ✅ Done 2026-05-09 — full plan closed same day | Plan archived: [`docs/10_planning/archive/19_library_transcode_followups.md`](./archive/19_library_transcode_followups.md).  M7 shipped as launch-priority commit `627cdf1` (global `streaming_mode` toggle, AV1/VP9 fmp4 stream-copy, migration 028).  M1-M6 + M8 shipped same-day via 2 parallel Opus subagents: M1 quality preset chooser (Smaller/Recommended/Mastering, default `cq=23`); M2 storage mode + configurable cache root (migration 029); M3 `/transcode/storage` endpoint + storage strip widget; M4 folder-grouped tree with tri-state checkboxes (Candidates + History); M5 Queue dialog + per-row source→sidecar size + stored-at menu; M6 cache-root validation + stale-sidecar detection (migration 031) + .webm→.mkv override + partial-output cleanup on crash recovery; M8 per-library AV1/VP9 overrides (migration 030, tri-state) + library-delete with sidecar-cleanup checkbox.  Server **734 → 775 (+41)**, desktop **104 → 113 (+9)**.  3 new migrations (029, 030, 031). |
| Library transcode (user-driven AV1/VP9 → H.264 sidecars) | Should | ✅ Done 2026-05-09 (M6 + M7 deferred) | Plan: [`docs/10_planning/18_library_transcode_plan.md`](./18_library_transcode_plan.md). Triggered by operator hitting the AV1-software-decode cliff (1× realtime → 404 storms on transcode-bound playback). Migration **027** adds `transcoded_path` / `transcoded_size_bytes` / `transcoded_at` to `media_files` + new `transcode_jobs` queue table. New service `services/transcode_service.py` runs a single FIFO worker that polls `queued` rows, builds an FFmpeg cmd (`h264_nvenc slow cq=19` if available, else `libx264 slow crf=19`), parses `-progress pipe:2` to update `progress_pct` + `eta_sec` in DB, and on success writes the sidecar columns on the source row. New router `/api/v1/transcode/*` (5 endpoints: candidates, queue, jobs, cancel, retry; bearer-or-localhost). Streaming pipeline reads `transcoded_path` if set + file exists on disk, else falls back to source with a warning log. Desktop `/transcode` route renders a 3-tab UI (Candidates multi-select / Queue with live progress / History with Retry on failed). Crash-recovery sweep marks orphan `running` rows as `failed` on app boot. **Test counts: server 698 → 730 (+32); desktop 90 → 104 (+14).** §9 design decisions resolved: H.264 only, default `slow cq=19` only, concurrency=1, AV1+VP9 only as candidates, copy everything, keep orphan transcodes, never auto-delete originals, side-by-side storage. **Deferred to v1.1:** M6 post-scan toast (non-blocking — Candidates is reachable from sidebar) + M7 stale-detection on rescan (manual rescan + source-mtime warning enough for v1). |
