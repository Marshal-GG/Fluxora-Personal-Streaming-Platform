...
| Library file-browser power features (plan 28) | Should | 📝 Drafted 2026-05-16 — 19 features ranked into Phase A-D + 4 tier-3 separate plans.  Replaces the MVP single-click-opens browser shipped same day with single/double-click semantics, right detail panel, sortable columns, grid view, search, type chips, keyboard nav, right-click menu, per-file Index/Re-thumbnail actions, back/forward history, multi-select, density toggle.  Plan: [`docs/10_planning/28_library_file_browser_power_features.md`](./28_library_file_browser_power_features.md). |
