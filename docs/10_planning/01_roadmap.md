...
| Android ExoPlayer migration (plan 24) | Must | 🔵 Active 2026-05-15 — replaces libmpv-on-Android (the source of repeat audio bugs — channel-mask emulation, AudioTrack churn, mid-stream `setAudioTrack` hang) with Media3 ExoPlayer via a hand-rolled Kotlin platform channel. Introduces `PlayerEngine` Dart abstraction in `packages/fluxora_core/lib/player/`; `MediaKitEngine` (desktop + iOS unchanged) + `ExoPlayerEngine` (Android, new). M1 platform-channel spike + M2 abstraction kicked off in parallel 2026-05-15. ~37 h ≈ 5 working days. Rollback via `_kForceMediaKitOnAndroid` flag. iOS deferred; multi-rendition HLS server output (industry-standard `#EXT-X-MEDIA TYPE=AUDIO` groups) split into plan 25. Plan: [`docs/10_planning/24_player_audio_reliability_plan.md`](./24_player_audio_reliability_plan.md). |
