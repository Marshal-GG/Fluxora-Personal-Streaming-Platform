...
| Library transcode (user-driven AV1/VP9 → H.264 sidecars) | Should | 🔲 Planned (drafted 2026-05-09) | Plan in [`docs/10_planning/18_library_transcode_plan.md`](./18_library_transcode_plan.md). Triggered by operator hitting the AV1-software-decode cliff (1× realtime → 404 storms on transcode-bound playback). Opt-in: post-scan toast surfaces candidates → desktop multi-select UI → server queue worker runs FFmpeg jobs → playback automatically uses the transcoded copy via stream-copy. Mobile changes: zero. 8 milestones (~10 h); M1-M4 backend-only, M5+ surfaces it. **8 design questions in §9 are gating M1**, awaiting user answers. |
