...
| Client-side audio decoding (plan 21) | Should | 🚧 Drafted 2026-05-12 — awaiting M1 sign-off | Plan: [`docs/10_planning/21_client_audio_decoding.md`](./21_client_audio_decoding.md).  Adds audio stream-copy for `{aac, ac3, eac3, opus, flac}` (FLAC + AC3/EAC3 surround preserved instead of being silently AAC-128k'd), fmp4 switch for non-AAC audio, separate `client_audio_codec_blocklist` (migration 034), new `POST /fallback-audio-transcode` endpoint for audio-only fallback on auto mode, 128k → 256k AAC bitrate bump + `-ac` channel preservation on the remaining re-encode paths.  ~12 h across M1-M5. |
