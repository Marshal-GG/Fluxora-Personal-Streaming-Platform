...
| Multi-audio-track support (plan 22) | Should | 🚧 Drafted 2026-05-14 — bandaid landed same day (`-map 0:v:0 -map 0:a:0?` in `_build_ffmpeg_cmd` so segments match the init.mp4 fallback's single-track contract; multi-track files play track 0). Plan 22 removes the pin, fixes `_ensure_fmp4_init_segment` to declare all tracks, adds `audio_tracks` JSON column + scan-time persistence (migration 035), exposes `AudioTrackInfo[]` on `StreamStartResponse`, builds the mobile Audio sheet picker with media_kit client-side track switching. See [`docs/10_planning/22_multi_audio_track_support.md`](./22_multi_audio_track_support.md). |
