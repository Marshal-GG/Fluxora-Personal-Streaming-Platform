...
| Android ExoPlayer migration (plan 24) | Must | 🔵 Active 2026-05-15 — **M1-M4 + M7 + M8 shipped same day (commits `62b2b21` spike, `bdffeb9` abstraction, `5db7e54` Dart engine, `575787e` Kotlin module, `0debbe9` Media3 MediaSessionService, `4ba2a87` seek/scrubber integration, `a1a9c12` plan-tag strip pass).** Replaces libmpv-on-Android (the source of repeat audio bugs — channel-mask emulation, AudioTrack churn, mid-stream `setAudioTrack` hang) with Media3 ExoPlayer 1.10.1 via a hand-rolled Kotlin platform channel (`MethodChannel('dev.marshalx.fluxora/exo_player')` + `EventChannel('dev.marshalx.fluxora/exo_player_events')`, both keyed by `playerId`). `PlayerEngine` Dart abstraction in `packages/fluxora_core/lib/player/`: `MediaKitEngine` (desktop + iOS unchanged) + `ExoPlayerEngine` (Android default as of M3+M4). `_kEnableExoPlayerEngine = true` post-M4; Android consumers construct `ExoPlayerEngine` by default; rollback via `_kForceMediaKitOnAndroid` flag. M7 native Media3 `MediaSessionService` owns lockscreen / notification card / BT transport for the ExoPlayer path; `FluxoraAudioHandler` stays bound for desktop + iOS + Android-rollback. M8 verified seek-restart + scrubber pin + progress reporter under ExoPlayer; closed two engine-cadence mismatches (`open()` state-reset emission + scrubber pin gate). **M5 multi-audio real-device smoke + M6 HDR+tonemap require operator device testing** (no agent work). **M9 tests + golden re-baseline + doc sweep + flag deletion + spike cleanup remain** (~4 h, mechanical close-out). iOS deferred; multi-rendition HLS server output (industry-standard `#EXT-X-MEDIA TYPE=AUDIO` groups) split into plan 25. Plan: [`docs/10_planning/24_player_audio_reliability_plan.md`](./24_player_audio_reliability_plan.md). |
