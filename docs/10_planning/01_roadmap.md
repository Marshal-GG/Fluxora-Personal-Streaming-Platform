...
| Library file-browser power features (plan 28) | Should | 🔵 In Progress 2026-05-16 — Phase A ✅ shipped (commits `65a3555` server + `f0971ae` client).  Server-side `_attach_index_status` rewritten as one LEFT-JOIN exposing per-indexed-entry `media` payload (width/height/duration_sec/codec_name/hdr_format/audio_codec/thumbnail_status/thumbnail_generated_at_unix/indexed_at_iso/is_streaming) + `mtime_unix` + stale-thumbnail auto-re-queue.  Client gains 320 px right detail panel, single-click-selects + double-click-opens, sortable column headers, list ↔ grid view with real thumbnails for indexed media, in-place search box, indexed-only toolbar toggle, HDR + ▶-live row pills, distinct empty-state copy.  Three new widget files (`LibraryBrowseDetailPanel`, `LibraryBrowseSearchBar`, `LibraryBrowseViewToggle`).  Server suite 916 → 925 (+9).  Phases B (filter chips + count footer + keyboard nav + polish) / C (right-click menu + editable path + per-file Index/Re-thumb endpoints + density + multi-select) / D (back/forward history + lazy folder-size) pending.  Plan: [`docs/10_planning/28_library_file_browser_power_features.md`](./28_library_file_browser_power_features.md). |
