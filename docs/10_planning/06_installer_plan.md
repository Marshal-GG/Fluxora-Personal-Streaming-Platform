...
| 4 | **Open Windows Firewall rules at install** | Without this, mobile clients can't reach the server on first install — pairing fails silently. Critical UX blocker. | `netsh advfirewall firewall add rule name="Fluxora Server" dir=in protocol=TCP localport=8080 action=allow` in post-install |
