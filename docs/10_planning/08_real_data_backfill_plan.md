...
> **Category:** Planning
> **Status:** Phases A + B landed 2026-05-04; Phase F (Trending) ripped 2026-05-08 with Browse-strip + Browse-chip replacement; Phases C / D / E still scheduled.
...
| **F — Trending** *(✅ ripped 2026-05-08)* | Trending-now rail removed entirely from Home + "Trending searches" chip group removed from Search empty-state. Single-tenant single-user trending is degenerate. Replacement landed in the same change: 4-up Browse strip on Home + Browse chip group on Search empty-state, both routing to `Routes.libraryWithFilter(slug)` which feeds `LibraryScreen.initialFilter` (mobile redesign plan §17.2). | None | `_MockRail` deleted; `_BrowseStrip` + `_BrowseTile` widgets render the 4-up category strip; "Trending searches" chip group replaced with `_browseFilters` "Browse" chips in Search empty-state; new `Routes.libraryWithFilter(slug)` helper. | `MockData.trending` + `MockData.trendingSearches` (both deleted) |
