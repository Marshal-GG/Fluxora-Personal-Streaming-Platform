...
> **Category:** Planning
> **Status:** Active — Updated 2026-05-08 (mobile redesign M10/M11/M12 + settings remediation plan M1–M5 + M2.5 server endpoint all closed; Profile-as-settings rebuilt from "8 of 11 dead-tap" to "8 live + 2 honestly-stubbed"; new bearer-only `PATCH /api/v1/auth/clients/me` self-rename endpoint; `connectivity_plus`-backed Wi-Fi-only enforcement in `PlayerCubit.startStream`; audit §17.3 #8 (Notifications FIFO cap parity) + #9-Custom (sleep-timer `showTimePicker`) closed in same round.  **Server suite 669; mobile suite 75; desktop 90; core 8.**  Settings remediation plan archived to [`docs/10_planning/archive/15_mobile_settings_remediation_plan.md`](./archive/15_mobile_settings_remediation_plan.md).  Audit §17.3 #1 (iOS PIP) + #7 (player-overlay goldens — folds into M14) + #9-End-of-episode (needs next-episode resolver) remain the only mobile-redesign opens.  M13 (host-a-server shell — Phase 5+ runtime gate) + M14 (goldens + a11y) are the only redesign milestones still open.) 2026-05-07 (mobile UI row flipped to ✅ — Groups v2 mobile shipped completes the redesign rollout)
