...
> **Category:** Planning
> **Status:** **Archived 2026-05-09** after full close-out (original location: `docs/10_planning/19_library_transcode_followups.md`). Drafted 2026-05-09. **Strategy pivot 2026-05-09 (same day):** v1 launches with client-side decoding as the **default streaming mode**. M7 shipped same day as launch-priority commit `627cdf1`. **M1, M2, M3, M4, M5, M6, M8 ALL ✅ shipped 2026-05-09** via 2 parallel Opus subagents — full plan 19 scope is closed. Server suite **734 → 775 (+41)**; desktop **104 → 113 (+9)**. **All 4 close-out sharp edges ✅ fixed 2026-05-10:** (1) `/transcode/storage` now returns `by_library` breakdown so the library-delete confirmation renders the actual N + GB; (2) `Expando`-keyed memoisation on `buildFolderTree` so 5000+-candidate trees don't recompute per build; (3) `ApiClient.delete` gained a `queryParameters: Map<String, dynamic>?` hook (desktop's `?delete_sidecars=` no longer hand-built); (4) `Process.start("explorer", ...)` replaced with `url_launcher`'s `launchUrl(Uri.file(path))` for clean cross-platform open-folder behaviour. Plan-19-and-followups now fully closed.
