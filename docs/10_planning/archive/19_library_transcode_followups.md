...
> **Category:** Planning
> **Status:** **Archived 2026-05-09** after full close-out (original location: `docs/10_planning/19_library_transcode_followups.md`). Drafted 2026-05-09. **Strategy pivot 2026-05-09 (same day):** v1 launches with client-side decoding as the **default streaming mode**. M7 shipped same day as launch-priority commit `627cdf1`. **M1, M2, M3, M4, M5, M6, M8 ALL ✅ shipped 2026-05-09** via 2 parallel Opus subagents — full plan 19 scope is closed. Server suite **734 → 775 (+41)**; desktop **104 → 113 (+9)**. Sharp edges flagged at close-out (per-library size breakdown in `/transcode/storage`; folder-tree memoisation; `ApiClient.delete` query-param hook; Windows `Process.start` exit-code wrap) live in the AGENT_LOG close-out entry — they're follow-up enhancements, not unfinished plan-19 work.
