...
> **Status: ✅ DONE 2026-05-14 — All M0–M14 milestones shipped. Mobile redesign fully closed.**  M14 landed 2026-05-14: animations to spec (scrim fade 250 ms, transport press 50 ms, ripple 400 ms, tab scale 220 ms, route fade 250 ms), Semantics labels on every interactive element, `FocusTraversalGroup` on form flows + player chrome, system text-scale clamped to 1.3×, 10 golden baselines at `apps/mobile/test/goldens/`. Mobile test suite: **92 passing** (82 unit/widget + 10 goldens). Earlier history: M0–M9 landed 2026-05-03 (V2-pure theme); Phase A + B real-data backfill 2026-05-04; QR-pairing scanner 2026-05-04; player polish (PIP + audio_service + bg toggle) 2026-05-04; seek-restart wire-up 2026-05-05; Groups v2 mobile UX shipped 2026-05-07; M10/M11/M12 shipped 2026-05-08; mobile-settings remediation plan M1–M5 + M2.5 server endpoint completed and archived 2026-05-08. Next milestones (not blocking redesign): **M13 host-a-server shell** (Phase 5+ runtime gate); remaining opens: iOS PIP (§17.3 #1) + end-of-episode resolver (§17.3 #9). Note: the prototype's "Group Watch" feature is a separate multi-client party-watch UI, NOT related to "client groups" / Groups v2 — both terms exist; don't conflate them. Audit completed 2026-05-08 — see §17 for findings + improvised suggestions. **Mobile-settings remediation plan (M1–M5 + M2.5 server endpoint)** completed and archived 2026-05-08 at [`docs/10_planning/archive/15_mobile_settings_remediation_plan.md`](../10_planning/archive/15_mobile_settings_remediation_plan.md) — Profile-as-settings rebuilt from "8 of 11 dead-tap" to "8 live + 2 honestly-stubbed", new `Routes.upgrade`/`Routes.account`/`Routes.privacy`/`Routes.playbackPrefs`, bearer-only `PATCH /api/v1/auth/clients/me` self-rename endpoint, `connectivity_plus`-backed Wi-Fi-only enforcement.
...
| `connectivity_plus: ^7.1.1` ✅ landed M3 settings remediation (2026-05-08) | Two consumers in one dep: (1) Wi-Fi-only streaming enforcement on the M3 Playback prefs screen — the player needs to sniff cellular vs. Wi-Fi before starting a stream when the pref is on; (2) the M10 Offline screen's live online/offline detector that flips the empty-state copy when a download is started while disconnected. Adding the dep to the screen-only mobile target keeps `fluxora_core` clean (no platform deps for the desktop control panel). | None — `dart:io.NetworkInterface.list()` answers "do I have an interface up" but cannot distinguish Wi-Fi from cellular, which is the actual question Wi-Fi-only enforcement asks. | ✅ locked. |
...
| **M12 — Onboarding revamp** ✅ done 2026-05-08 | New `apps/mobile/lib/features/onboarding/presentation/screens/splash_screen.dart` matches the prototype `SplashScreen` (`docs/11_design/prototype/app/mobile/screens/splash.jsx`) — 104-px glow `FluxoraMark` + `FluxoraWordmark` + "Stream. Sync. Anywhere." tagline + 3-dot pagination + two CTAs. **Scope adapted to Fluxora's auth model:** the prototype's three CTAs ("Sign in" / "Connect to a server" / "Continue as guest") collapse to two real paths — primary `Connect to a server` → `Routes.connect` and secondary `Scan a QR code` → `Routes.scanQr`. The "Sign in" CTA from the prototype is dropped — Fluxora has no credential auth, only operator-approval pairing, so a separate "sign in" path would just route to the same place. "Continue as guest" is dropped — the app cannot operate without a paired server. New `Routes.splash = '/splash'`; auth-gate `_guardRedirect` now sends unauthenticated users to `/splash` (was `/connect`); authenticated users on `/splash` are bounced to `/home`; `initialLocation` flipped to `/splash`. **Server picker (`connect_screen.dart`) rebuilt:** `FluxAppBar` + eyebrow/h1/body header block + `BrandLoader` while scanning + glass `_ServerTile`s with `LucideIcons.server` + bottom CTAs as `FluxButton` (Scan QR primary + Enter manually secondary opening a `FluxBottomSheet` with `FluxTextField` IP/Port form + Connect CTA + per-field error). Old plain-`ElevatedButton` chrome retired. **Pairing screen (`pairing_screen.dart`) polished:** two `CircularProgressIndicator` instances (inline 28-px loader + 22-px pending-poll spinner) replaced with the branded `BrandLoader` (56 px / 32 px) so first-time pairers see Fluxora chrome on every state, not generic Material spinners. Other pairing surfaces (email-collection / pending / rejected / error) were already V2-styled and kept untouched. The original M12 row's "email + password + 2FA TOTP + invite-code paths" scope was **honestly cut** — Fluxora is single-tenant + self-hosted; there is no credential auth, no TOTP, no invite codes. Adding placeholder UI for a backend that doesn't exist would be feature-creep. The signin rebuild collapses into the splash + connect + pairing-polish work above. `flutter analyze` clean × `apps/mobile`; 64 mobile tests still pass. | M2 |
...
| **M14 — Polish + a11y + golden tests** ✅ shipped 2026-05-14 | Wave 1a — 29 Semantics nodes across `FluxPlayerControls` + `FluxMiniPlayer`; animation constants: scrim fade 250 ms `easeOut`, seek ripple 400 ms `easeOut` (stateful `_SeekRipple` with own controller), transport press 50 ms scale 1.0↔0.92, peek badge + drag HUD AnimatedSwitcher/AnimatedOpacity 250 ms; focus traversal: `FocusTraversalGroup(OrderedTraversalPolicy)` with `NumericFocusOrder` on siblings (1 top-bar → 2 transport → 3 brightness-rail → 4 volume-rail → 5 progress-bar → 6 quick-actions); autofocus on play/pause. Wave 1b — text-scale clamp 1.3× at app root; tab-scale animation 220 ms; route-fade 250 ms on every `GoRoute`; Semantics on all core widgets; `FocusTraversalGroup` on form flows; ~30–35 Semantics wrappers across 16 screens. Wave 2 — `golden_toolkit: ^0.15.0` added; 5 private widgets renamed public (`PlayerTopBar`, `PlayerCenterTransport`, `PlayerProgressBar`, `PlayerQuickActions`, `PlayerSideRail`); **10 golden tests** (top bar / center transport / progress bar / side-rail-left / side-rail-right / lock overlay / mini player / bottom sheet / app bar / poster); 10 Windows baselines at `apps/mobile/test/goldens/goldens/`. **Test counts: mobile 82 + 10 goldens = 92 total; 25/25 player cubit; 8/8 core; 10/10 goldens.** `flutter analyze` clean × mobile + core. | All prior milestones |
...
onboarding/   splash_screen.dart        ← M12 ✅ landed 2026-05-08
```

**§M12 onboarding scope adapted (2026-05-08):** Fluxora has no credential auth, only operator-approval pairing.  The prototype's three-CTA splash (Sign in / Connect / Continue as guest) collapses to two real CTAs (Connect to a server / Scan a QR code) — both lead to the same pair flow.  The "signin_screen.dart" + "server_picker_screen.dart" files in the prototype port don't exist in Fluxora — that work folds into the existing `features/auth/presentation/screens/pairing_screen.dart` (V2-polished at M12 — `BrandLoader` swapped in for the inline spinners; the email-collection / pending / rejected / error panels were already V2-styled at landing) and `features/connect/presentation/screens/connect_screen.dart` (rebuilt at M12 — `FluxAppBar` + `BrandLoader` while scanning + glass `_ServerTile`s + bottom CTAs as `FluxButton` + manual entry as a `FluxBottomSheet`).
...
| `features/connect/presentation/screens/connect_screen.dart` | ✅ rebuilt at M12 (2026-05-08) on V2 tokens — `FluxAppBar` + `BrandLoader` + glass `_ServerTile`s + bottom CTAs as `FluxButton` + manual entry as a `FluxBottomSheet` with `FluxTextField`. **Not** renamed to `server_picker_screen.dart` — keeping the existing path avoids churning every router import + test reference for cosmetic gain. | M12 |
| `features/auth/presentation/screens/pairing_screen.dart` | ✅ polished at M12 (2026-05-08) — two `CircularProgressIndicator` instances replaced with `BrandLoader` (56 px / 32 px); other panels (`_EmailEntryPanel`, `_PendingPanel`, `_ErrorPanel`) were already V2-styled.  **Not** renamed to `signin_screen.dart` — Fluxora has no "signin" concept (only operator-approval pairing); the existing path is the honest description. | M12 |
...
✅ **All criteria met — redesign closed 2026-05-14.**

- ✅ All M0–M14 milestones merged.
- ✅ `flutter analyze` green for `apps/mobile` and `packages/fluxora_core`.
- ✅ `flutter test apps/mobile` green — **92 passing** (82 unit/widget + 10 goldens; was 25 at plan-draft time).
- ✅ Golden images per overlay component: `top_bar`, `transport`, `progress_bar`, `side_rail_left`, `side_rail_right`, `lock_overlay`, `mini_player`, `bottom_sheet`, `app_bar`, `poster` — 10 baselines at `apps/mobile/test/goldens/goldens/`.
- ⏸ Manual QA on a real Android device + a real iOS device — Android confirmed on device; iOS PIP pending iOS test device (§17.3 #1 open, does not block redesign close).
- ✅ `docs/00_overview/current_status.md`, `docs/08_frontend/01_frontend_architecture.md`, `docs/10_planning/01_roadmap.md` updated.
- ✅ This plan's "Status" line at top updated to ✅ Done — merged 2026-05-14.
...
| 2026-05-08 | **M12 Onboarding revamp landed.** New `apps/mobile/lib/features/onboarding/presentation/screens/splash_screen.dart` (104-px glow `FluxoraMark` + wordmark + "Stream. Sync. Anywhere." tagline + 3-dot pagination + two CTAs — primary `Connect to a server` → `/connect`, secondary `Scan a QR code` → `/scan-qr`).  New `Routes.splash = '/splash'`; auth-gate `_guardRedirect` now sends unauthenticated users to `/splash` (was `/connect`) and bounces authenticated users off `/splash` to `/home`; `initialLocation` flipped to `/splash`; `pairingFlows` set in the `unauthorizedStream` listener also includes `/splash` so a token-revocation event mid-onboarding doesn't reroute on top of itself.  Server picker `connect_screen.dart` rebuilt — `FluxAppBar` ("Find your server") + header block (eyebrow / h1 / body) + `BrandLoader` while scanning + glass `_ServerTile`s with `LucideIcons.server` + bottom CTAs as `FluxButton` (Scan QR primary + Enter manually secondary opening a `FluxBottomSheet` with `FluxTextField` IP + port form + Connect CTA + inline validation error).  Old plain-`ElevatedButton` chrome retired.  Pairing screen `pairing_screen.dart` polished — two `CircularProgressIndicator` instances replaced with `BrandLoader` (56 px / 32 px) so first-time pairers see Fluxora chrome on every state, not generic Material spinners.  Email-collection / pending / rejected / error panels were already V2-styled and kept untouched.  The original M12 row's "email + password + 2FA TOTP + invite-code" scope was **honestly cut** — Fluxora is single-tenant + self-hosted; there is no credential auth, no TOTP, no invite codes; adding placeholder UI for a backend that doesn't exist would be feature-creep.  Plan §7 M12 / §17.1 M12 / §17.4 #6 all flipped to "✅ landed".  `flutter analyze` clean × `apps/mobile`; 64 mobile tests still pass. |
| 2026-05-14 | **M14 landed — Mobile redesign closed.** Animations to spec (scrim fade 250 ms easeOut, transport press 50 ms scale 1.0↔0.92, ripple 400 ms easeOut with stateful `_SeekRipple`, peek badge + drag HUD AnimatedSwitcher/AnimatedOpacity 250 ms). Semantics labels on every interactive element (~29 nodes in player chrome + ~30–35 across 16 screens). `FocusTraversalGroup(OrderedTraversalPolicy)` on player chrome (top-bar→transport→rails→progress→quick-actions) + form flows (connect IP field, pairing email). System text-scale clamped to 1.3× via `MediaQuery.withClampedTextScaling` in `app.dart`. Tab-scale animation 1.0→1.05 at 220 ms easeOut. Route-fade 250 ms / 200 ms reverse on every `GoRoute`. 10 golden baselines at `apps/mobile/test/goldens/goldens/` covering all 10 plan-promised components. `golden_toolkit: ^0.15.0` added to pubspec. Private-widget exposure pattern: `_TopBar`→`PlayerTopBar`, `_CenterTransport`→`PlayerCenterTransport`, `_ProgressBar`→`PlayerProgressBar`, `_QuickActions`→`PlayerQuickActions`, `_SideRail`→`PlayerSideRail`. Mobile suite **82 → 92 (+10 goldens)**. |
...
| M12 Onboarding revamp | pending (QR scanner partly done) | ✅ landed 2026-05-08 | Splash + server-picker rebuild + pairing polish all shipped — see §7 M12 row. The original "email + password + 2FA TOTP" scope was honestly cut; Fluxora has no credential auth so there is nothing to mock. |
| M13 Host-a-server shell | pending | pending | UI shell only; runtime is Phase 5+. |
| M14 Polish + a11y + golden tests | pending | ✅ landed 2026-05-14 | Animations to spec (50/250/400/220 ms constants), Semantics on all elements, focus traversal, text-scale clamp 1.3×, 10 golden baselines. Mobile test count 82 → 92. Redesign fully closed. |
...
7. ~~**No goldens for the player overlay yet.**~~ ✅ Closed 2026-05-14.  `golden_toolkit: ^0.15.0` added to mobile `pubspec.yaml` (matches desktop pin).  5 private player widgets renamed public with `@visibleForTesting`: `_TopBar`→`PlayerTopBar`, `_CenterTransport`→`PlayerCenterTransport`, `_ProgressBar`→`PlayerProgressBar`, `_QuickActions`→`PlayerQuickActions`, `_SideRail`→`PlayerSideRail`.  10 baselines at `apps/mobile/test/goldens/goldens/` (Windows; CI excludes via `--exclude-tags=golden`).  Shared mock helper at `apps/mobile/test/goldens/_player_mocks.dart` (mocktail-based `MockPlayer`).  `_README.md` documents the private-widget exposure convention + `await GetIt.I.reset()` async gotcha + 5 sharp edges.  See `apps/desktop/test/goldens/_README.md` for the source recipe that mobile ported.
8. ~~**Notifications panel — 500-entry FIFO cap is mobile too?**~~ ✅ closed 2026-05-08. Mobile [`features/notifications/data/repositories/notifications_repository_impl.dart`](../../apps/mobile/lib/features/notifications/data/repositories/notifications_repository_impl.dart) `liveStream()` now caps `seen` at 500 with FIFO eviction (`seen.remove(seen.first)` when `seen.length > _seenCap`); pulls `_pollLimit = 20` per tick — both constants mirror the desktop client's pattern. Doc-comment cross-references the desktop file so future drift between the two implementations is obvious.
9. **Sleep-timer — "Custom…" ✅ + "End of episode" still stubbed.** Custom landed 2026-05-08: `Custom…` row in [`sleep_sheet.dart`](../../apps/mobile/lib/features/player/presentation/sheets/sleep_sheet.dart) now opens `showTimePicker` (24-h mode forced via `MediaQuery(alwaysUse24HourFormat: true)` so the AM/PM toggle doesn't confuse a duration dial-in) — output `TimeOfDay(hh, mm)` is reinterpreted as `Duration(hours: hh, minutes: mm)` with `0:00` treated as cancel. When a custom duration is active, the row label flips to `Custom (1h 30m)` style and renders selected. Default seed is `0:45` (most common sleep-window for this UI shape). **End of episode still 🔲** — needs the next-episode resolution that hangs off `PlayerCubit`'s end-of-stream hook; lands as one unit with autoplay-next (settings remediation §M3 follow-up) so the resolver isn't designed twice.
...
6. ~~**M12 — Onboarding revamp**~~ ✅ landed 2026-05-08 — splash + server-picker rebuild + pairing polish (the original "email/password/TOTP/invite" scope was cut as inapplicable to Fluxora's pairing-only auth model; see §7 M12 row).
...
After M12 closing, the only redesign milestones still open are **M13** (host-a-server shell — Phase 5+ runtime) and **M14** (polish + a11y + golden tests).
