...
| M10 X-Ray + Group Watch + Offline | pending | pending | Highest priority next mobile work — see §17.4. |
| M11 Beyond-video viewers | pending | pending | Blocked on `pdfx` / `photo_view` / `just_audio` selection at execution time. |
...
2. **Profile data is hardcoded.** `/api/v1/profile` is the operator profile, not a per-paired-client surface. **Suggested fix:** add `GET /auth/clients/me/profile` that returns `{display_name, avatar_url, email_or_local_id, plan_badge, hours_watched, movies_watched, shows_watched}` so the Profile hero stops lying. Prefer the new endpoint to repurposing the operator one.
3. **Sign-out flow does not revoke the bearer server-side.** `apiClient.clearBearerToken()` + `secureStorage.deleteAll()` happen on the client only — the server still treats the token as valid until expiry. **Suggested fix:** call `POST /auth/sessions/me/revoke` before clearing local state. Server route exists for desktop sign-out; mobile should reuse it.
4. **Continue-watching rail has no empty state.** Falls back to mock when the endpoint returns `[]`. **Suggested fix:** render a single CTA card ("Pick something to watch" → routes to Library) instead of mock fixtures.
5. **`background_gradient.dart` paints two `RadialGradient`s on every routed screen.** On low-end Android the cumulative cost compounds with player + glass blur + poster image decode. **Suggested fix:** wrap the gradient stack in a `RepaintBoundary` (the gradient itself is static — repaints cost nothing if the boundary is never invalidated).
6. **`mobile_scanner` + `pdfx` + `photo_view` + `just_audio` + `audio_service` versions in §6 still say "check pub.dev at MX".** **Suggested fix:** at the start of M11, run a one-shot version sweep and lock the table — five separate `flutter pub outdated` lookups is too easy to skip later.
...
2. **M10 — X-Ray panel + Group Watch shell + Offline state.** This is the next planned milestone. Group Watch is shell-only per §1 row 4; X-Ray uses static metadata; Offline is a single empty-state screen. Estimated 1–2 sessions.
3. **Profile real-data endpoint** (§17.3 #2). Backend + mobile + desktop. Maybe 1 session if we keep the response narrow.
4. **iOS PIP** (§17.3 #1). Big enough that it's its own ticket — flag it but don't start until 2–3 are done.

After 2–4 land, M11–M14 can resume in plan order.
