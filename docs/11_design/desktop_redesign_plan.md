...
### M5 — Groups + Activity + Transcoding + Encoder Settings *(✅ Done 2026-05-02 — Groups subsequently rebuilt by plans 12 + 13 + 14 in 2026-05-07; see those plans for the v2 content-spaces redesign + dedicated edit page)*

- **New entities** in `packages/fluxora_core/lib/entities/`: `Group` / `GroupRestrictions` / `TimeWindow` / `GroupStatus` (`group.dart`); `TranscodingStatus` / `EncoderLoad` / `ActiveTranscodeSession` (`transcoding_status.dart`). Both freezed + json_serializable.  v2: `Group` extended with `isPublic` / `requiresPin` / `pinMode` / `pinModel` / `icon` / `color` / `maxConcurrentStreams`; `PinMode` + `PinModel` enums added.
- **New endpoints** in `Endpoints`: `groups`, `groupById(id)`, `groupMembers(id)`, `groupMember(groupId, clientId)`, `transcodingStatus`.  v2 added: `groupEnter`, `groupEnroll`, `groupEnrollChange`, `groupGrant`, `groupGrantStatus`, `groupMemberPin`, `groupGrantsReset`, `authClientVisibleLibraries`, `authClientsMeVisibleLibraries`.
- **Groups feature** (`apps/desktop/lib/features/groups/`): `GroupsRepository` + `GroupsRepositoryImpl`; `GroupsCubit` + `GroupsState`; full `GroupsScreen` — `PageHeader`, 4 `StatTile`s, table with selected-row highlight, 300px detail panel with restrictions + members list, create/edit/delete dialogs, add/remove member.  **Subsequently extended 2026-05-07 with a dedicated 6-tab edit page at `/groups/new` + `/groups/:id/edit` (Overview / Members / Access / PIN / Activity / View As); legacy create/edit modal retired; shared form widgets lifted to `widgets/group_form_widgets.dart`.  Plan: [`docs/10_planning/14_groups_management_page.md`](../10_planning/14_groups_management_page.md).**
...
| 2026-05-07 | Claude (session) | **Tier 2 benchmark axis shipped: resolution-tier matrix.** Encoder Settings → Benchmark tab's resolution chip row converted from single-select to multi-select. Operator picks any subset of {720p, 1080p, 4K} and the benchmark runs each tier sequentially through every encoder, producing `len(encoders) × len(resolutions)` rows. Server: `BenchmarkRequest.resolutions: list[ResolutionTuple] \| None` additive over the existing `width`/`height` (legacy clients keep working); new `clamp_resolutions(pairs)` snaps + dedupes preserving order; `EncoderBenchmarkResult` (dataclass + Pydantic) gains required `width` + `height`; `run_benchmark` outer loop per-resolution + inner per-encoder; `_progress` global + `BenchmarkProgress` Pydantic gain `total_resolutions` / `current_resolution_index` / `current_resolution_width` / `current_resolution_height`; `BenchmarkResponse.resolutions` echoes the tested list; `BenchmarkHistoryEntry.resolution_count` (defaults to 1 for legacy rows) derived server-side from the persisted blob's distinct `(width, height)` tuples. `get_benchmark_run` back-fills per-row width/height for legacy rows from the run's primary dimensions. No migration. Desktop: new `Resolution` freezed entity; `_ResolutionSelector` reworked to multi-select toggle chips; `_BenchmarkBlock._selectedResolutions: Set<…>` defaults to `{1080p}` so first-run UX is unchanged; Run gated on non-empty; sequential walk in canonical tier order (720p first); `_BenchmarkResultsTable` sorts within each vendor section by (encoder, resolution-pixels) — operator sees `h264_nvenc[720p, 1080p, 4K]` then `hevc_nvenc[...]`; matrix rows get a violet resolution `FluxChip` next to the codec pill; `_SourceWorkloadCaption` collapses to a tier list when multi; `_BenchmarkProgressCard` tags the encoder with its current tier ("Encoding h264_nvenc [1080p]") + adds "Res N of M" caption; `_HistoryEntryRow` flips to "N res" caption when `resolutionCount > 1`. `ApiClient` receive timeout 6 min → 12 min in matrix mode. Tests: +8 server cases (clamp/dedupe, N×M ordering, per-row width/height self-description, progress threading, router accepts list body + echoes, list-precedence over legacy width/height, history `resolution_count` for matrix + legacy). Server suite **565 → 573 passing**. `flutter analyze` clean across desktop + core. |
