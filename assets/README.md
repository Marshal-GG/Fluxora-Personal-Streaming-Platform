...
| Mobile Android (launcher icon) | `apps/mobile/android/app/src/main/res/mipmap-{m,h,xh,xxh,xxxh}dpi/ic_launcher.png` | Resize `assets/brand/logo-icon.png` (1254×1254 RGBA) with LANCZOS to 48 / 72 / 96 / 144 / 192 px and overwrite each `ic_launcher.png`. The master is rendered as-is — its dark gradient bg is part of the brand mark and the launcher expects the icon to fill the canvas. Re-run any time `logo-icon.png` is replaced. |
| Mobile iOS (app icon) | `apps/mobile/ios/Runner/Assets.xcassets/AppIcon.appiconset/Icon-App-*.png` | Resize the same master to every `(size × scale)` pair declared in `Contents.json` (16 PNGs total — 20/29/40/60/76/83.5 px at 1×/2×/3× plus 1024×1024 marketing). Filenames in `Contents.json` are the source of truth — read it before regenerating. |
