...
Fluxora is a self-hosted media streaming product. Most installs run on a single home machine, but the server can be exposed to the public internet via the Cloudflare Tunnel feature, and bearer tokens / license-key HMAC secrets / paired-device credentials all live on the operator's hardware. Vulnerabilities in any of those surfaces matter — even though the product is closed-source, the binary is on user hardware and is reachable from the network. We treat coordinated disclosure the same way an open-source project would.
...
Vulnerabilities in any component shipped to end users:

- **Fluxora server binary** — auth/pairing flow, license-key HMAC, FFmpeg subprocess handling, request validation, log redaction, the Cloudflare Tunnel ingress middleware.
- **Desktop control panel** (Windows / macOS / Linux).
- **Mobile clients** (iOS / Android).
- **Public landing site** at `fluxora.marshalx.dev`.
- **Build / release tooling** (signed-installer pipelines, release artifact integrity).
...
| Asset | Control |
|-------|---------|
| Bearer auth tokens | Stored as HMAC-SHA256 hashes only — never plaintext. Token is shown to the client once at approval and never re-derivable from the database. |
| License keys | HMAC-SHA256 signed; format `FLUXORA-<TIER>-<EXPIRY>-<NONCE>-<SIG>`. The server validates the signature locally with a secret bound to the install — no network call after the one-time Polar redemption at the moment of purchase. |
...
| Admin endpoints | Localhost-only on the server (rejects loopback callers carrying `CF-Connecting-IP` so a tunneled request cannot impersonate localhost). |
| HLS segments over the public tunnel | Blocked at the ingress middleware — segments are LAN-only by design. |
| Path traversal / unsafe upload paths | Validation in the scan + upload paths rejects relative, `[`-prefixed, and null-byte paths. |
| Logged sensitive data | Bearer tokens, passwords, license keys, customer email, and filesystem paths under the user's home directory are never logged in production builds. |
